# 畅聊 ChatFlow UI 设计规范

**版本：** 1.2
**生效日期：** 2026-08-20
**最近修订：** 2026-09-19（新增 §19 列表与卡片分割线统一为渐隐分割线；§6、§11.2、§12.1、§12.2 同步；新增 §20 钱包充值/提现页 2026-09-18 修订）
**适用范围：** Flutter Android/iOS 客户端
**设计语言：** 微信式信息架构与交互，保留“畅聊”独立品牌

**品牌文案：** 产品显示名统一为“畅聊 ChatFlow”；受空间限制的应用内文案可使用“畅聊”；账号标识使用“畅聊号”；需要品牌限定时，朋友圈显示为“畅聊朋友圈”，点钻红包显示为“畅聊点钻红包”。内部包名、Matrix 标识、资产代码和现有资源文件名保持不变。

## 1. 基本原则

1. 信息优先：减少装饰，使用清晰层级、稳定位置和即时反馈。
2. 双端一致：Android 与 iOS 使用同一套 Flutter 视觉组件，系统能力与权限流程可按平台适配。
3. 安全边界不变：主题与组件不得改变 Matrix E2EE、业务 API 权威状态、账本、红包和钱包安全规则。
4. 无障碍：支持系统字号、语义标签、对比度与减少动态效果。
5. 组件优先：业务页面不得自行硬编码颜色、字号、间距、圆角或动画时长。

## 2. 颜色变量

| Token | 浅色 | 深色 | 用途 |
|---|---:|---:|---|
| `brandPrimary` | `#07C160` | `#07C160` | 图标、选中态、徽标、成功态 |
| `brandPressed` | `#06AD56` | `#06AD56` | 品牌图标按压态 |
| `pageBackground` | `#EDEDED` | `#111111` | 页面底色 |
| `surfacePrimary` | `#F7F7F7` | `#191919` | 导航栏、输入区、卡片 |
| `surfaceElevated` | `#FFFFFF` | `#232323` | 弹窗、浮层、列表单元 |
| `textPrimary` | `#191919` | `#F5F5F5` | 标题、正文 |
| `textSecondary` | `#888888` | `#B2B2B2` | 描述、时间戳 |
| `textTertiary` | `#B2B2B2` | `#7A7A7A` | 占位、禁用文本 |
| `divider` | `#D9D9D9` | `#2C2C2C` | 渐隐分割线的色源（几何与透明度见 §19） |
| `danger` | `#FA5151` | `#FA5151` | 错误、取消、未读标记 |
| `warning` | `#FA9D3B` | `#FA9D3B` | 等待、红包 |
| `bubbleOutgoing` | `#95EC69` | `#3A6B36` | 自己的消息气泡 |
| `bubbleIncoming` | `#FFFFFF` | `#2C2C2C` | 对方消息气泡 |
| `scrim` | `#80000000` | `#99000000` | 弹层遮罩 |

禁止在业务页面使用上述 Token 之外的硬编码颜色。资产状态颜色只能通过语义 Token 选择，不得用颜色作为唯一状态信号。

### 2.1 强制页面配色映射

下表是浅色模式的颜色基准（2026-09-07 用户要求修订）。深色模式导航栏解析为 `#191919`、页面背景为 `#111111`、卡片/输入区为 `#232323`、正文为 `#F5F5F5`、分隔线为 `#2C2C2C`。所有自绘表面和文字须在 build 中按当前主题解析，浅色/深色/跟随系统切换应即时重绘，不得以本表浅色色值覆盖深色主题。照片、视频、二维码及品牌色保留自身颜色。

| 页面/区域 | 必用颜色变量 | 浅色色值 | 禁止替代 |
|---|---|---:|---|
| 私聊消息页顶部导航栏 | `chatNavigationBackground` | `#F7F7F7` | `surfaceElevated`、`pageBackground`、品牌色或自定义灰色 |
| 群聊消息页顶部导航栏 | `chatNavigationBackground` | `#F7F7F7` | `surfaceElevated`、`pageBackground`、品牌色或自定义灰色 |
| 私聊消息页整体背景 | `chatPageBackground` | `#EDEDED` | 白色、`surfacePrimary`、图片背景或自定义灰色 |
| 群聊消息页整体背景 | `chatPageBackground` | `#EDEDED` | 白色、`surfacePrimary`、图片背景或自定义灰色 |
| “消息”主导航根页面 | `tabRootPageBackground` | `#EDEDED` | `surfaceElevated`、白色或自定义灰色 |
| “通讯录”主导航根页面 | `tabRootPageBackground` | `#EDEDED` | `surfaceElevated`、白色或自定义灰色 |
| “发现”主导航根页面 | `tabRootPageBackground` | `#EDEDED` | `surfaceElevated`、白色或自定义灰色 |
| “我”主导航根页面 | `tabRootPageBackground` | `#EDEDED` | `surfaceElevated`、白色或自定义灰色 |

`WeChatAppTheme` 必须导出 `chatNavigationBackground`、`chatPageBackground` 和 `tabRootPageBackground`；`WeChatPageScaffold`、私聊页、群聊页及四个主导航根页消费这些变量时必须按当前主题解析。消息气泡、列表单元、输入区和浮层仍按其专用 Token 取色，不得反向改变页面整体或导航栏背景。

## 3. 字体与字号

默认字体使用平台系统字体；中文优先 PingFang SC（iOS）与 Noto Sans CJK/系统字体（Android），不捆绑字体文件。

| Token | 字号 | 字重 | 行高 | 用途 |
|---|---:|---:|---:|---|
| `display` | 28 | 600 | 36 | 余额、大数字 |
| `title1` | 22 | 600 | 30 | 大标题 |
| `title2` | 18 | 600 | 25 | 导航标题、弹窗标题 |
| `body` | 17 | 400 | 24 | 消息、主要正文 |
| `callout` | 16 | 400 | 22 | 按钮、列表标题 |
| `subhead` | 14 | 400 | 20 | 描述、状态 |
| `caption` | 12 | 400 | 17 | 时间戳、辅助说明 |
| `badge` | 11 | 500 | 14 | 未读数量 |

正文最低 14sp；支持系统文字缩放 0.85–1.4，资产数字采用等宽数字特性。

## 4. 间距与圆角

间距基于 4dp 网格：`xs=4`、`sm=8`、`md=12`、`lg=16`、`xl=24`、`xxl=32`。

圆角：

- 小标签与输入附件：4dp
- 输入框与现代操作按钮：14dp
- 消息气泡与列表卡片：8dp
- 红包卡片：10dp
- 底部操作面板：12dp（仅顶部）
- 弹窗：12dp
- 胶囊按钮：高度的一半

阴影只用于浮层：`0 4 16 rgba(0,0,0,0.12)`；普通列表和卡片不使用阴影。

## 5. 组件尺寸

- 顶部导航栏：44dp + 安全区；标题 17sp/600。
- 底部 TabBar：49dp + 安全区；图标 24dp，文字 10sp。
- 列表单元：最小 56dp；聊天会话单元 72dp。
- 头像：会话 48dp，消息 40dp，详情 64dp。
- 主按钮：高度 48dp，左右边距 16dp，圆角 14dp，白底细边框与轻阴影。
- 次按钮：高度 40dp。
- 文本输入区：最小 52dp，输入框最小 36dp。
- 点击区域：最小 44×44dp。
- 消息气泡：最大宽度为视口 72%，水平内边距 12dp、垂直 9dp。
- 语音条：高度 42dp，宽度随时长在 80–220dp 之间变化。
- 红包卡片：宽 236dp，高 96dp；金额与祝福语不得泄露到 Matrix 明文元数据。
- 弹窗：宽 280dp；操作按钮最小高 48dp。

## 6. 复用组件清单

| 组件 | 职责 | 关键参数 |
|---|---|---|
| `WeChatAppTheme` | 亮暗主题与语义 Token | brightness |
| `WeChatPageScaffold` | 页面背景、导航、安全区 | title, actions, child |
| `ModernActionButton` | 白底图文主/次/危险操作和加载态 | icon, label, kind, loading, onPressed |
| `WeChatGradientDivider` | 列表与卡片统一的渐隐分割线（见 §19） | height, indent, endIndent |
| `WeChatListTile` | 统一列表单元 | leading, title, subtitle, trailing, showDivider |
| `WeChatMessageBubble` | 文本消息及发送状态 | direction, content, status |
| `WeChatTimestamp` | 时间分组 | timestamp |
| `WeChatUnreadBadge` | 未读计数 | count, muted |
| `WeChatVoiceBubble` | 时长、波形、播放态 | duration, samples, playback |
| `WeChatRedPacketCard` | 红包卡片和状态 | greeting, state, packetId |
| `WeChatAttachmentTile` | 图片/文件上传状态 | name, progress, retry |
| `WeChatComposer` | 文本、附件、语音输入 | onSend, onPick, recorder |
| `WeChatStatusChip` | 钱包/提现状态 | semanticStatus |
| `WeChatDialog` | 确认、错误、详情 | title, content, actions |
| `WeChatToast` | 非阻塞反馈 | message, semanticType |
| `WeChatEmptyState` | 空状态与重试 | title, action |

所有组件位于 `apps/mobile_flutter/lib/ui/`，业务页面只能通过公开构造函数使用。组件支持 `key`、语义标签和必要的样式扩展，但不允许覆盖品牌与资产状态色。

## 7. 核心组件行为

### 消息

- 自己的消息靠右并使用 `bubbleOutgoing`，对方消息靠左并使用 `bubbleIncoming`。
- **发送状态文案只有五个词（2026-09-19 统一，2026-09-19 二次修订气泡视觉，禁止各页自造）**：`queued` 等待发送、`sending` 发送中、`waitingNetwork` 等待网络、`failed` 发送失败、`sent` 正常（正常态不显示任何标记）。
  - 对应实现：Outbox 状态机 `lib/core/outbox/outbox_message.dart`（`OutboxStatus.label`，五个词逐字一致）；会话内气泡 `lib/ui/chat/wechat_message_bubble.dart`（`MessageDeliveryState`）。
  - 气泡（2026-09-19 用户修订）：未发出的消息必须**及时给出红色感叹号警告**——`waitingNetwork`（网络原因暂未发出）与 `failed`（服务端明确拒绝）视觉一致：红色感叹号 + 点击立即重发。区别在行为：`waitingNetwork` 网络恢复后自动重发（感叹号随之消失），`failed` 仅点击重发。「等待发送」保留给本地排队待发（`queued`）的行与未打开会话的列表文案；`sending` / `sent` 不显示多余标记（发送中不增加加载感知）。
  - 弱网/断网的发送失败必须在有界时间内可见（发送护栏 20s；离线时立即快速失败），不允许气泡长期停留在"无标记"状态。
  - 未打开的会话页（pending conversation / outbox 列表）直接显示上述五词中的当前状态文案。
- 发送中与已发送均不显示多余标记（2026-09-19 修订：不增加用户加载感知）；失败显示红色重试图标。
- 连续 5 分钟内的消息不重复显示时间戳；跨日显示完整日期。
- 未读标记最多显示 `99+`。

### 语音

- 按住开始；上滑超过 60dp进入取消区；松开发送；取消区松手丢弃。
- 少于 1 秒不发送并给出轻提示；单条上限 60 秒。
- 录音页显示秒数和 20–24 段归一化波形。
- 录制结束先进入本地试听状态，由用户发送或删除；不得自动上传。

### 附件

- 图片：JPEG/PNG/WebP，单文件上限 20MiB。
- 文件：允许列表由配置定义，默认 PDF/ZIP/TXT/DOCX/XLSX，单文件上限 100MiB。
- 权限按需请求；拒绝时显示设置入口。
- 上传展示 0–100% 进度；失败保留本地选择并允许重试。
- 明文只存在于设备内存/临时文件；发送走 Matrix SDK 的加密媒体路径，不进入业务 API。

### 红包

- 使用 `warning` 橙色卡片，底栏显示“畅聊点钻红包”。
- 状态包括：可领取、已领取、已领完、已过期、已撤回。
- 卡片 Matrix 事件只含红包 ID 和非敏感展示字段；金额、领取人和权威状态从业务 API 获取。

### 钱包

- 交易记录支持全部/充值/提现筛选。
- 状态：处理中、成功、失败；同时显示文字与图标。
- USDT 固定六位小数；地址默认中间省略，详情页才可复制完整地址。

## 8. 动效

- 按压反馈：80ms 降至 0.72 透明度，120ms 恢复。
- 页面转场：220ms 横向推入；返回手势可中断。
- 弹窗：180ms 淡入并从 0.96 缩放到 1.0。
- 消息发送：160ms 从输入区平移并淡入，不采用弹跳。
- 下拉刷新：跟手移动；触发后最短展示 300ms。
- 红包弹层：220ms 缩放淡入。
- 上传进度：线性更新，不对伪进度做循环动画。
- 系统开启减少动态效果时，取消位移、缩放和波形动画，只保留不超过 100ms 的淡入淡出。

## 9. 页面结构

底部导航固定为：消息、通讯录、发现、我。点钻、红包、钱包入口置于“我”和聊天业务卡片，不再作为四个并列资产 Tab；首轮迁移可暂保留资产入口，但最终必须收敛到上述信息架构。

登录、注册和邮箱验证页使用 `landing.png` 沉浸背景、半透明白色输入控件和白底图文操作按钮；错误显示在对应字段或表单下方并提供重试，不显示技术异常文本。

## 10. 使用与审查

- 新组件必须有 Widget 测试、亮暗主题快照或结构断言、无障碍语义断言。
- 代码审查搜索硬编码颜色、间距和字号；除 Token 定义外不得新增。
- 每次 UI 改动同时更新本文件和 `docs/verification/` 证据。
- 本文件优先于旧 Cupertino 主题规范；架构与安全规范仍以产品现代化设计为准。

### 10.1 配色规范检查清单

- [ ] `WeChatAppTheme` 中定义并导出 `chatNavigationBackground=#F7F7F7`、`chatPageBackground=#EDEDED`、`tabRootPageBackground=#EDEDED`。
- [ ] 私聊和群聊消息页顶部导航栏只引用 `chatNavigationBackground`，截图/Widget 测试断言其色值为 `#F7F7F7`。
- [ ] 私聊和群聊消息页的根 Scaffold/消息列表背景只引用 `chatPageBackground`，截图/Widget 测试断言其色值为 `#EDEDED`。
- [ ] “消息、通讯录、发现、我”四个主导航根页只引用 `tabRootPageBackground`，截图/Widget 测试断言其色值均为 `#EDEDED`。
- [ ] 新增或修改相关页面时，不得使用 `Color(0xFF...)`、`Color.fromARGB(...)`、十六进制字符串或第三方默认背景色绕过上述变量。
- [ ] 代码审查同时检查嵌套 `Scaffold`、`Container`、`CustomScrollView`、空状态和加载状态，确保它们没有覆盖根页面或导航栏规定的背景色。
- [ ] 本节八个区域必须支持浅色→深色→浅色及跟随系统即时切换；深色导航/页面不能残留浅色；变更同步 Token、页面测试及设计登记。

## 11. 通讯录组件与页面

### 11.1 信息架构

“通讯录”作为底部第二个 Tab，导航顺序固定为：新的朋友、群聊、标签、公众号/官方客服、按拼音分组的好友列表。右侧索引条宽 20dp，字号 10sp；当前索引使用 `brandPrimary`。

### 11.2 通讯录组件

| 组件 | 规格 |
|---|---|
| `WeChatContactTile` | 高 56dp，40dp 头像，昵称 16sp，行底共享渐隐分割线（整行宽；组内最后一行的线由分组标题承担，见 §19） |
| `WeChatContactIndex` | 右侧 A–Z 索引，触摸反馈显示 64dp 居中浮层 |
| `WeChatFriendRequestTile` | 高 68dp，状态按钮 64×32dp，支持接受/拒绝/已添加 |
| `WeChatContactTagChip` | 高 28dp，圆角 4dp，浅灰背景 |
| `WeChatPrivacyRow` | 朋友圈权限、黑名单及时间范围设置行 |

好友主页包含头像、展示名、畅聊号、签名、朋友圈预览、发消息及音视频入口。备注、标签、删除、拉黑及朋友圈权限统一置于右上角“更多”页面，破坏性操作使用 `danger`。

## 12. 朋友圈 UI

### 12.1 入口与时间线

朋友圈入口位于“发现”Tab 首组第一行，使用 24dp 相机/图片图标。存在新内容时，右侧显示最近发布者 32dp 头像和红点。

时间线顶部为 280dp 封面区：封面铺满、底部渐变遮罩、右下角显示 72dp 用户头像和白色昵称。列表背景使用 `surfaceElevated`，单条动态水平边距 12dp，左侧头像 42dp，正文区域与头像间距 10dp。动态与动态之间使用共享渐隐分割线（§19），不再使用实心边框。

### 12.2 动态组件

| 组件 | 规格 |
|---|---|
| `WeChatMomentTile` | 发布者、正文、媒体、位置、时间、操作菜单及互动区；行底共享渐隐分割线（§19） |
| `WeChatMomentImageGrid` | 1 张限制 240×240dp；2–4 张双列；5–9 张三列；间距 4dp |
| `WeChatMomentActionMenu` | 深色 `#4C4C4C` 面板，高 40dp，点赞/评论各 80dp |
| `WeChatMomentReactions` | `#F7F7F7` 背景，点赞与评论区，区块之间为共享渐隐分割线，圆角 4dp |
| `WeChatMomentVisibilityIcon` | 12dp，表示公开、好友、部分可见、不给谁看、仅自己 |
| `WeChatMomentComposer` | 正文、九宫格、位置、提醒谁看、可见范围 |
| `WeChatMomentNotificationTile` | 48dp 头像、互动摘要、动态缩略图和时间 |

正文使用 16sp/23sp 行高；发布者名称使用 `#576B95`，该蓝色仅作为社交用户名与可点击文本 Token `socialLink`。位置、删除和时间使用 13sp。

### 12.3 发布页

- 纯文字或文字加 1–9 张图片；首版不展示视频入口。
- 正文输入区最小高 120dp，无外框。
- 图片单元按容器三等分，删除按钮为右上角 20dp 深色圆形。
- 底部设置项：所在位置、提醒谁看、谁可以看。
- 发布按钮 64×32dp；内容为空时禁用。
- 上传中离开页面需要确认；失败图片保留本地预览并显示重试。

### 12.4 详情、搜索与推荐

动态详情复用 `WeChatMomentTile`，下方完整展示点赞与评论。搜索页采用顶部搜索框和正文、用户、话题、位置、时间筛选；无权限内容不得短暂出现。

公开时间线顶部使用“推荐/最新”双段选择器，高 32dp；关闭个性化推荐后默认进入“最新”。推荐标记不得伪装成好友动态。

### 12.5 朋友圈设置

设置项包括：允许陌生人查看、朋友圈时间范围（全部、最近半年、最近一个月、最近三天）、不让他看、屏蔽他的朋友圈、关闭个性化推荐。时间范围选择使用底部操作面板并立即显示当前值。

### 12.6 内容治理状态

发布状态包括上传中、审核中、已发布、部分可见、已下架和发布失败。审核中使用灰色状态条；下架使用红色说明并提供申诉入口。好友内容和公开内容均接受基础安全检测，公开推荐仅包含审核通过内容。

## 13. 认证页面

- `liuhetong_logo.svg` 是 Android/iOS APP 图标唯一源文件。
- `landing.png` 是登录、注册和邮箱验证页统一背景。
- `ImmersiveAuthScaffold` 使用全屏 `cover`，焦点保持在背景上部品牌主体；键盘出现时只移动表单层。
- 登录字段：用户名/邮箱、密码；提供注册入口。
- 注册字段：用户名、邮箱、密码、强制邀请码；邀请码为空时提交不可用。
- 邮箱验证支持 6 位验证码、验证链接、60 秒重发与注册状态反馈。

### 13.1 激活码验证与使用状态

- 界面将邀请码展示为“激活码”；其合法性、使用次数和状态以业务 API 为唯一权威来源，客户端不得依据本地显示状态将其标记为已使用。
- 激活码仅在注册用户**通过激活码成功完成注册**时标记为“已使用”。成功定义为注册服务的创建用户、建立注册会话和激活码消费均在同一业务事务内提交成功；提交成功后才允许返回注册成功状态。
- 注册流程中的信息校验失败、网络错误、请求超时、服务端拒绝、邀请码失效/耗尽、邮箱验证未完成、创建账号失败或任何其他失败，均不得将激活码标记为“已使用”，也不得减少剩余使用次数。
- 激活码状态变更必须置于注册成功事务之后：事务提交失败必须回滚激活码消费和使用次数；在消费动作之前失败时状态保持不变。客户端重试使用同一幂等键时只能重放同一成功结果或同一失败结果，不得发生额外消费。
- 注册失败提示必须保留可重试的激活码输入，不显示“已使用”或其他与实际权威状态不一致的本地状态。

## 14. 现代操作按钮

| 类型 | 视觉 |
|---|---|
| 主操作 | 白底、1dp 中性边框、轻阴影、绿色图标、深色文字 |
| 次操作 | 透明背景、图标加文字 |
| 危险操作 | 白底、红色图标、红色文字 |
| 导航操作 | 44×44dp 点击区内的单一图标 |

禁止新增大面积纯绿填充按钮。本规范适用于认证、好友、朋友圈、点钻、红包、钱包和设置页面。按下即时缩放至 0.98；松开使用临界阻尼恢复；减少动态效果时仅改变透明度或颜色。

## 15. 用户资料与头像

- `UserIdentityHeader` 展示 72dp 头像、昵称、畅聊号和个性签名。
- 本人头像支持相册选择、正方形裁剪、预览、上传、修改和恢复默认头像。
- 默认头像使用昵称首字符与稳定浅色背景。
- 好友头像 48dp，资料页头像 72dp；加载失败必须回退默认头像。
- 邮箱只在本人资料页显示脱敏值。

### 15.1 聊天成员头像缓存

- 私聊和群聊消息页必须使用统一的 `AvatarCache`/头像提供器加载会话成员头像；页面不得直接为每个可见成员反复发起网络请求。
- 首次进入私聊或群聊时，先查内存缓存，再查磁盘缓存；两级均未命中时才请求网络。网络成功后同时写入内存和磁盘缓存，并以用户 ID、头像版本（或规范化头像 URL）和尺寸作为缓存键。
- 同一聊天页再次进入、列表滚动复用或应用前后台切换时，命中内存/有效磁盘缓存必须直接显示本地资源，避免重复网络加载；磁盘命中可先显示缓存内容，再后台按更新策略检查。
- 内存缓存采用 LRU，至少容纳最近 200 个头像条目；内存压力时可逐出但不得影响磁盘缓存。磁盘缓存有效期为 30 天，单条缓存使用服务器 `ETag`/头像版本和最后验证时间；过期条目可显示陈旧缓存并后台刷新，网络失败时继续显示陈旧缓存或默认头像。
- 头像版本或 URL 变更、个人资料上传/修改头像成功、联系人资料同步事件及服务器 `ETag` 变化，必须精确失效该用户的内存和磁盘条目，并拉取/写入新资源；不得清空无关成员的缓存。用户主动清除应用缓存时清除磁盘缓存，内存缓存随进程生命周期清除。
- 群聊成员头像按成员用户 ID 独立缓存；成员加入、离开或昵称变化不得使其他成员头像重新下载。加载中显示稳定默认头像，不得造成列表布局跳动。
- 缓存仅保存已授权展示的缩略头像字节和必要的版本/有效期元数据；不得把 Matrix 消息明文、访问令牌或其他用户资料写入头像缓存。

## 16. 好友主页与更多

- 通讯录展示名优先级：备注名 > 昵称 > 用户名；绝不显示内部 UUID。
- 好友主页主操作为白底图文“发消息、语音通话、视频通话”。
- 右上角 `ellipsis` 进入更多页，包含备注、标签、朋友圈权限、黑名单和删除好友。
- 发消息复用现有 E2EE Direct Chat；创建中显示不可重复点击的加载态。

## 17. “我”页面

- 顶部必须展示个人资料卡，点击头像或资料区进入编辑页。
- 功能组顺序：朋友圈、点钻、红包、钱包、设置。
- 设置页显式包含退出登录，使用白底危险操作行和二次确认。

## 18. 网络状态胶囊

`NetworkStatusCapsule` 仅在离线/重连时悬浮于导航栏下方，不占据整行，不推动页面主体或 TabBar。使用半透明材质、网络图标和短文案；点击触发立即重试，恢复联网后自动淡出。


## 16. 共享锚定操作菜单（2026-09-10）

入口统一调用 `showAnchoredActionMenu<T>`，渲染统一使用 `WeChatAnchoredActionMenu<T>`（`lib/ui/components/anchored_action_menu.dart`）。聊天气泡的 `MessageBubbleMenu` 只负责消息能力及顺序映射，委托相同组件；业务页面不得复制菜单容器样式。

| 规范 | 固定值 / 行为 |
|---|---|
| 表面 | ARGB `#E64C4C4C`，白色图标/文字 `#FFFFFF`；在白底合成约 `#5E5E5E`，白字对比度约6.5:1 |
| 尺寸 | 宽272dp，圆角8dp；横向内距6dp，纵向4dp；每行最多4项 |
| 图标/文字 | 图标20dp，字号11sp，间距3dp；至少44dp触摸高度；允许系统字号及长文案换行 |
| 行分隔 | 0.5dp，白色24%透明度；不足4项时保留同一列宽 |
| 指示箭头 | 11dp方形旋转45°，颜色同表面，指向触发元素；靠屏幕上方时改为上箭头 |
| 出现动画 | 120ms，透明度0→1、缩放0.96→1；系统减少动画时立即显示 |
| 遮罩与关闭 | `#1A000000`；点击外部、返回或选项后关闭；动作在菜单关闭后执行 |
| 定位 | 触发元素锚点，避让安全区、键盘与屏幕边缘；空间不足允许菜单内滚动 |

用途与菜单内容由业务上下文决定：

- 消息气泡：沿用消息能力判断，复制、转发、收藏、引用、提醒、撤回、多选、删除按可用性展示。
- 本人朋友圈下的好友评论：长按提供“复制”“删除”；单击仍为回复。其他人的动态不得显示删除他人评论。服务端以登录身份和动态/评论作者校验权限，失败保留原评论并提示。
- 消息/通讯录/发现右上角“更多”：使用 16.2 的纵向页面菜单；聊天气泡和会话长按继续使用本节网格。
- 会话长按：置顶该聊天（已置顶则取消置顶）、标记未读、不显示该聊天、删除该聊天。隐藏保留内容，新消息后重新显示；删除必须确认，只影响本设备本账号的历史呈现，不退出群聊、不清除其他成员数据。新消息到达后仍可继续该聊天。

HTML设计参考对应 `app-anchored-action-menu`，Token前缀 `--anchored-menu-*`；Flutter数值入口 `AnchoredMenuTokens`。Figma沿用既有消息气泡组件节点作为设计参照，新菜单变体的远端同步状态单独记录在导出台账，未同步前不得声称远端已更新。

### 16.1 群视频发送反馈

原始视频上限20MB（20×1024×1024字节，等于上限允许）。相册、文件、拍摄入口统一在压缩/读取整文件前拦截超限视频，提示“视频大小不能超过20MB”，不通过压缩绕过原始大小约束。上传/加密状态显示在消息任务中，不禁用文字输入、滚动和菜单；失败保留可重试状态。后台计算保持端到端加密及账号生命周期隔离。

### 16.2 顶部更多菜单与会话反馈（2080 基线修订）

三个 Tab 统一调用 `showTopMoreMenu`，顺序固定为“发起群聊、添加朋友、扫一扫、外观”。每项独占一行，整行可点击；常规行高52dp，宽208dp，圆角8dp，左右内距16dp，图标20dp、字号16sp、图文间距12dp。分隔线0.5dp、白色24%，左缩进48dp、右12dp。表面与文字沿用16节；120ms淡入/缩放，减少动态效果时取消动画。大字号允许行高增加，屏幕不足可滚动，三个页面始终复用同一组件。

置顶和手动未读应在点击后的下一帧更新，不能等待网络同步。所有置顶会话（含私聊）位于普通会话之前；按 pinnedAt 倒序，最新置顶在最上方。置顶背景复用 navigationSurface：浅色 #EDEDED、深色 #191919，普通会话为 #FFFFFF / #232323。状态须账户隔离并持久保存；取消置顶恢复普通排序。手动未读与正常未读复用同一红色气泡，实际进入该会话才清除。

长按底部消息图标使用系统 mediumImpact 触觉反馈，缩放1→0.8→1，时长300ms，减少动态效果时取消缩放。无论当前 Tab，均立即清除全局与会话未读显示；操作期间重复长按不创建并发清理，之后的新消息正常显示未读。

退出登录保留原确认，随后显示本机记录处理弹窗。默认强调“保存”；“确认删除”必须显式点击，红色粗体；取消不执行退出或删除。弹窗先关闭再调用账户生命周期操作，保存不清理记录；删除调用账户级本地清理后退出。所有失败状态不能卡住操作或误报删除成功。

## 19. 列表与卡片分割线（渐隐分割线，2026-09-19 生效）

列表行之间、列表与卡片之间，以及卡片内部相邻区块之间的**水平分割线**统一使用同一个共享渐隐分割线，禁止第二套实现：

- Flutter：`WeChatGradientDivider(height, indent, endIndent)`（`apps/mobile_flutter/lib/ui/components/wechat_gradient_divider.dart`）；
- HTML 设计演示：`app-divider` 组件（`frontend/src/components/divider.js`，根 class `.c-gradient-divider`），或行容器上的 `--divider-fade` 背景（`.c-list-tile`、`.c-contact-row` 的既有写法）。

几何与不透明度固定为：两端 alpha 0、中段 alpha 0.5、stops 0 / 0.18 / 0.82 / 1、色源 `divider` token、行高不因画线改变。

| Flutter token | HTML token | 值 | 用途 |
|---|---|---:|---|
| `WeChatDividerTokens.hairline` | `--size-hairline` | 1.0（1px） | 分割线高度（1 逻辑像素） |
| `WeChatDividerTokens.edgeStartStop` | `--divider-fade` 第 1 个色标 | 0.0（0%） | 左端点 |
| `WeChatDividerTokens.coreStart` | `--divider-fade-core-start` | 0.18（18%） | 中段起点（0 → 18% 由透明渐入） |
| `WeChatDividerTokens.coreEnd` | `--divider-fade-core-end` | 0.82（82%） | 中段终点（82% → 100% 再渐出到透明） |
| `WeChatDividerTokens.edgeEndStop` | `--divider-fade` 第 4 个色标 | 1.0（100%） | 右端点 |
| `WeChatDividerTokens.edgeAlpha` | `--divider-fade-edge-alpha` | 0.0 | 两端不透明度（完全透明） |
| `WeChatDividerTokens.centerAlpha` | `--divider-fade-alpha` | 0.5 | 中段不透明度（低于实心分割线的 1.0） |
| `WeChatColors.divider`（色源） | `--color-divider`（配合 `--divider-fade`） | `#D9D9D9` / `#2C2C2C` | 浅色 / 深色取色 |

**硬性约束**

1. 禁止新增第二套实心分割线实现：业务页面不得自行拼 `Container` + `Border(bottom:)`、`ColoredBox(height: 1)`，HTML 不得再写 `border-bottom: 1px solid var(--color-divider)` 来画列表/卡片分隔线；需要画线时改用上述共享组件。
2. 深浅色均按主题解析：Flutter 在 build 时经 `WeChatColors.resolve(context, WeChatColors.divider)` 解析（浅色 `#D9D9D9`、深色 `#2C2C2C`），HTML 由 `.ui-screen` 与 `.ui-screen[data-theme="dark"]` 上的 `--divider-fade` 解析。禁止硬编码浅色值。
3. 行高不因画线改变：分割线画在行内（Flutter 行内底部 `Positioned`/`Column` 尾项，HTML `background-position: bottom center` + `background-size: 100% var(--size-hairline)`），不额外占用行高；字母索引偏移等依赖行高的几何不得变化。
4. 缩进只在确有设计缩进时使用 `indent` 参数（例如朋友圈可见范围页的 16dp），默认整行宽。
5. 组件、token 与使用点必须同时出现在 `packages/ui-contracts/changliao-component-registry.json`（`gradient-divider`）与 HTML demo，Flutter 与 HTML 的 token 值逐项一致。

**允许保留实心细线的例外（逐条理由）**

| 位置 | 保留理由 |
|---|---|
| 导航栏 / TabBar 的上下细线（HTML `.c-navigation-bar`、`.c-tab-bar`；Flutter `CupertinoNavigationBar.border`） | 属于导航 chrome 的边缘线，不是列表或卡片的行分隔线；两端渐隐会让导航区与内容区失去边界 |
| 弹窗内容与操作区之间的线及其竖线（HTML `.c-dialog__actions`、`.c-dialog__button--confirm`） | 弹窗为固定两列按钮，横线必须与中间竖线成对，渐隐会让竖线悬空 |
| 输入类控件与按钮自身的描边（`Border.all`：`.c-form-field__input`、`.c-search-field__input`、`.c-action-button` 等） | 是控件轮廓而不是分隔线 |
| 深色浮层菜单的行分隔线（Flutter `TopMoreMenuTokens.divider`，HTML `.c-anchored-menu__item` 的 `--anchored-menu-divider`） | 浮层自带深色专用 token（白色 24%）；共享分割线取 `divider` token（浅灰），在该深色表面上不可读 |
| 管理后台表格与卡片（`frontend/src/styles/admin-*.css`、`gallery.css` 的 `--admin-border`） | 管理后台使用独立的 `--admin-*` token 体系与密集数据表格；表格横竖线必须成对，不属于移动端设计演示的组件体系 |
| 图片编辑器底部控制条（HTML `.c-image-editor__sheet .c-image-editor__control`） | 工具条控件行（不是列表行），其上下边界是控件边缘 |


---

## 20. 钱包充值 / 提现页（2026-09-18 修订）

本节是 `lib/features/wallet/manual_wallet_page.dart`（充值页 / 提现页）与
`lib/features/caibi/caibi_page.dart`（点钻页进入态）的硬性规范；未在此登记的
视觉一律沿用 §2 颜色、§5 间距、§6 共享组件与 §19 分割线约束，不新增 token。

### 20.1 进入态：缓存优先 + 后台刷新（2026-09-18 用户报告「按钮闪烁 → 错误提示 → 数据恢复」）

| 规则 | 说明 |
|---|---|
| 数据源 | 能力配置 + 点钻余额（钱包页）/ 余额 + 最近流水 + 本月汇总（点钻页）合并为一份快照，由 `lib/features/finance/wallet_entry_store.dart` 的 `WalletEntryStore` 持有（共享实例按 `钱包作用域#会话 epoch` 复用） |
| 进入即渲染 | 有缓存时**先渲染缓存**（余额不得先变 `—` 再恢复），刷新在后台进行；不得因为刷新让整页进入 busy 禁用态 |
| 只有一种失败可以弹错 | `WalletEntryState.fatalError`（从未成功过且没有任何数据可展示）。有缓存的刷新失败必须保留旧值，不得出现红色错误条/错误弹窗/禁用闪烁；`lastError` 只允许做弱提示 |
| 生命周期 | 页面只 `view.addListener/removeListener`，**不得** dispose 共享 Store（持有者是会话级宿主） |
| 作用域隔离 | 页面与点钻页必须使用不同 scope（点钻页为 `<钱包作用域>#caibi`），否则两页会共享同一份结构不同的快照；账号/会话变化时金融数据立即丢弃，绝不跨账号展示 |

### 20.2 地址与订单码展示（压缩 + 左对齐 + 复制完整值）

| 规则 | 说明 |
|---|---|
| 地址展示 | 一律 `compactWalletAddress`：首 8 位 + `…` + 末 6 位；长度不足时原样返回。禁止用 `TextOverflow.ellipsis` 硬截断地址 |
| 对齐 | 地址行与 `detail()` 单元格同一水平内边距（`WeChatSpacing.lg`）：label 定宽 72dp 左对齐、值列左对齐、复制 icon 固定在行右端；因此不同长度的地址不会让 icon 错位 |
| 复制语义 | 复制 icon 始终复制**完整**地址/订单码（展示值只用于看），复制后给可见反馈 |
| 订单展示码 | 提现的订单校验码/订单号直接展示（不再折叠在「查看详情」按钮后），字号 `WeChatTypography.caption`，并带复制 icon；绑定版本等次要字段作为普通明细行 |

### 20.3 表单、余额与步骤条

| 规则 | 说明 |
|---|---|
| 表单间距 | 输入框与同一行的动作按钮（如「全部提现」）之间至少 `WeChatSpacing.md`（12dp），不得相贴 |
| 余额层级 | 点钻余额 hero：标签 `WeChatTypography.subhead` + `textSecondary`；余额数字 `WeChatTypography.brand`（34）w700 + 主题主文本色；单位 `callout` + `textSecondary`；说明 `caption` + `textTertiary`。深色下必须解析为深色主题色；超长金额用 `FittedBox(scaleDown)`，不得溢出 |
| 金额可改 | 确认提现之前金额输入框始终可编辑（不得设为只读/禁用）；修改后旧报价不得再展示或提交，必须按最终输入重新向服务端报价；「全部提现」与输入框联动一致 |
| 步骤条 | 点「下一步」后步骤指示器**保留**：已完成步骤 = 品牌色 + 勾号，当前步骤 = 品牌色，未完成 = 表面色 + 次级文字；进度变化用 `AnimatedContainer` 过渡，时长取 `WeChatMotion.actionPressDuration`，系统「减少动态效果」开启时归零 |
| 有效期 | 提现确认有效期只展示服务端权威 `expires_at`，本地过期校验使用同一个时间点；前端不得自行编造时长（现行 5 分钟是服务端策略 `wallet_manual_quote_ttl_seconds`，改成 24 小时属服务端改动） |
| 状态唯一性 | 确认提现后同一笔只以「处理中」订单呈现一次：确认订单（报价卡）在同一笔已生成订单后不再渲染 |

### 20.4 申请通知栏（顶部导航栏下方）

| 规则 | 说明 |
|---|---|
| 位置 | 「查看已有充值申请 / 查看已有提现申请」统一放在钱包页**顶部导航栏下方的固定通知栏**（不随列表滚动，且不再散落在卡片下方） |
| 交互 | 点击整条直达对应申请页（恢复同一申请，不做二次确认）；最右侧是「不再通知」图形 icon（`CupertinoIcons.bell_slash`） |
| 忽略语义 | 忽略标记按**申请身份**持久化（`deposit:<申请 id>`、`payout:<报价 id/订单 quote_id>`），不是内存 bool：进程重启后仍有效，出现**新的一笔**申请（身份不同）时提醒重新出现；保存失败必须可见，不得假装成功 |
| 视觉 | 品牌淡底 `WeChatColors.brandTint` + `WeChatRadius.bubble` 圆角，正文用主题主文本色、图标用 `brandPrimary`；深浅色都正确；窄屏下文字单行省略 |
| 保存到本地 | 收款二维码的「保存到本地」是带图形 icon 的动作控件：申请相册写入权限（`ensureGallerySaveAccess`）→ 渲染白底静区 PNG → 写入系统相册；进行中显示 loading 且禁用，成功/失败都有可见文案，无权限必须给出可读原因（不得静默失败） |
