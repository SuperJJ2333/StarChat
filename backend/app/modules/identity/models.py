...
class ReferralInvite(Base):
    """用户当前窗口的 referral 邀请码发布表（每用户一行，读取时滚动更新）。

    码本身由 HMAC 按 30 分钟窗口确定性推导，不落明文；此表仅存 sha256，
    供校验接口按码 O(1) 反查邀请人（unique 索引），窗口不匹配即视为旧码失效。
    """
...
class ReferralBinding(Base):
    ...
    code_window_index: Mapped[int] = mapped_column(BigInteger, nullable=False)
    status: Mapped[str] = mapped_column(String(20), nullable=False, default="ACTIVE")
    reward_state: Mapped[str] = mapped_column(
