...
class ReferralValidateRequest(StrictModel):
...
def create_identity_router(
    ...
    *,
    ...
    referral_codec = ReferralCodec(
        secret=settings.referral_code_secret
        or "development-referral-code-secret",
        rotation_seconds=settings.referral_rotation_seconds,
    )
    referral_service = ReferralService(session_factory, codec=referral_codec)
    ...
    async def validate_invitation(body: InvitationRequest, request: Request) -> dict:
        ...
        reason = normalize_invitation_reason(
    ...
    async def get_referral_code(
        ...
        """当前用户专属推荐码（30 分钟轮换，客户端按 rotates_at 实时刷新）。"""
        user_id = claims["sub"]
        rate_limiter.hit(
            f"referral:current:{user_id}", limit=200, window_seconds=1800
        )
        result = referral_service.current_code(user_id)
        share_url = (
            f"{settings.referral_share_base_url}"
            f"?code={quote(result['code'], safe='')}"
        )
        return {
            "code": result["code"],
            "rotates_at": result["rotates_at"],
            "rotates_in_seconds": result["rotates_in_seconds"],
            "share_url": share_url,
            "reward_enabled": settings.referral_reward_enabled,
        }

    @router.post("/invitations/referral/validate")
    async def validate_referral_code(body: ReferralValidateRequest, request: Request) -> dict:
        """公开校验推荐码：只回答有效/无效，不回显邀请人（防枚举）。

        双层限流：针对"IP+码"窄桶（单码爆破）与针对 IP 宽桶（批量枚举）。
        """
        source_ip = request.client.host if request.client else "unknown"
        rate_limiter.hit(
            public_rate_limit_key(
                "referral:validate:code", source_ip, body.referral_code
            ),
            limit=10,
            window_seconds=60,
        )
        rate_limiter.hit(
            public_rate_limit_key("referral:validate:ip", source_ip),
            limit=30,
            window_seconds=3600,
        )
        return {"valid": referral_service.peek(body.referral_code)}
    ...
    async def register(
        ...
        if result.referral_bound:
            record_audit(
                ...
                actor_id=result.user_id,
                subject_id=result.user_id,
                action="identity.referral.bound",
                reason_code="REFERRAL_BINDING",
            )
        return {
...
(async () => {
  ...
  status.textContent = response.ok
    ...
    async def presence_heartbeat(
        ...
        # TokenService refreshes device activity only when rotating refresh
        # tokens. Heartbeats keep foreground users visible without exposing
        # Matrix content or storing payloads.
        device_key = request.headers.get("X-Device-Key")
        now = datetime.now(timezone.utc)
        with session_factory.begin() as session:
            query = select(Device).where(Device.user_id == claims["sub"], Device.revoked_at.is_(None))
            if device_key:
                query = query.where(Device.device_key == device_key)
            device = session.scalar(query.order_by(Device.last_seen_at.desc()))
