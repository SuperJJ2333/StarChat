...
class RequestBody(Strict):target_user_id:str=Field(min_length=1,max_length=36);message:str=Field(default='',max_length=200);remark:str|None=Field(default=None,max_length=20);tags:list[str]=Field(default_factory=list,max_length=5);moments_permission:str=Field(default='DEFAULT',max_length=30)
...
def create_friendship_router(settings:Settings,factory,*,avatar_storage,rate_limiter=None):
    ...
    def request(body:RequestBody,idempotency_key:Annotated[str,Header(alias='Idempotency-Key')],user=Depends(actor)):
        if any(len(tag) > 64 for tag in body.tags):
            raise AppError(code='VALIDATION_ERROR',message='单个标签不能超过 64 个字符',status_code=422)
        r,duplicate=service.request(user,body.target_user_id,body.message,idempotency_key,remark=body.remark,tags=body.tags,moments_permission=body.moments_permission)
        return {'id':r.id,'status':r.status,'duplicate':duplicate}
