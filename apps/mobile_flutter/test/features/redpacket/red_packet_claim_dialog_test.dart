...
import 'package:liuhetong_mobile/core/notification/notification_feedback.dart';
...
void main() {
  ...
  testWidgets('领取后播放开启音并直接进入领取详情', (tester) async {
    final played = <SoundType>[];
    NotificationFeedback.install(played.add);
    addTearDown(NotificationFeedback.uninstall);
    ...
    expect(played, [SoundType.redpacketOpen]);
    // 弹窗关闭（居中磨砂弹窗样式不变），直接进入「领取详情」页。
  ...
  testWidgets('session invalidation tears down the claim entry',
      (tester) async {
    ...
    await tester.pumpWidget(CupertinoApp(
        home: RedPacketClaimDialog(api: gateway, packetId: 'packet-1')));
    await tester.pumpAndSettle();
    expect(find.byKey(const Key('red-packet-claim-luck-entry')), findsOneWidget);
    ...
    expect(find.byKey(const Key('red-packet-claim-open-button')), findsNothing);
  ...
  testWidgets('claim callback still fires and opens detail when refresh fails',
      (tester) async {
    var detailCalls = 0;
    var notified = 0;
    final gateway = FakeRedPacketViewGateway(detail: _openDetail);
