...
void main() {
  ...
  test('different Matrix identity reports both identities without clearing',
      () async {
    final business = FakeDualDomainBusiness();
    final matrix = FakeMatrixTokenLogin(isLoggedIn: true)
    ...
    await expectLater(
        ...
        throwsA(isA<MatrixAccountSwitchRequired>()
            .having((error) => error.fromMxid, 'fromMxid',
                '@bob:matrix.example.test')
            .having((error) => error.toMxid, 'toMxid',
  ...
  test(
    ...
    final service = DualDomainLoginService(
        business: business, matrix: matrix, deviceKey: () => 'device-1');
    await expectLater(service.login('alice', 'business-password'),
        throwsA(isA<MatrixAccountSwitchRequired>()));
  ...
  test('changed token target aborts confirmed switch before deletion',
      () async {
    final business = FakeDualDomainBusiness()
