...
void main() {
  ...
  late bool cancelled;
  Future<void> Function()? pendingAccept;
  Future<void> Function(String)? pendingNative;
  setUp(() {
    ...
    bridge = IosCallCoordinator(
      ...
      invoke: (method, args) async {
        native.add(method);
        if (pendingNative != null) await pendingNative!(method);
        return null;
      },
      accept: () async {
        if (pendingAccept != null) await pendingAccept!();
        answers++;
      },
      end: () async {
        ends++;
  ...
  test('end during native presentation prevents later answer admission',
      () async {
    final presented = Completer<void>();
    pendingNative = (method) async {
      if (method == 'showIncoming') await presented.future;
    };
    state = incoming('a');
    final accepting = bridge.handle('event', action('answer'));
    await Future<void>.delayed(Duration.zero);
    final ending = bridge.handle('event', action('end'));
    presented.complete();
    await accepting;
    await ending;
    expect(answers, 0);
  ...
  test('native end invalidates an answer before pending claim completes',
      () async {
    final claim = Completer<void>();
    pendingAccept = () => claim.future;
    state = incoming('a');
    final accepting = bridge.handle('event', action('answer'));
    await Future<void>.delayed(Duration.zero);
    final ending = bridge.handle('event', action('end'));
