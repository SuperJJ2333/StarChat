...
void main() {
  test('BUG-11 回归：拉黑联系人的消息不产生通知', () async {
    // 投影存的是业务 ID ↔ Matrix ID 双视角；同步层过滤按 Matrix ID 判定。
    blockedContacts.replaceAll(['u-bob'],
        fromServer: true, matrixUserIds: {'@peer:example.test'});
    addTearDown(blockedContacts.clear);
    ...
    final source = MatrixNotificationEventSource(client: client, now: () => now);
