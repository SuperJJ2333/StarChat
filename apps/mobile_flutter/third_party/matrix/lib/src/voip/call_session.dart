...
class CallSession {
  ...
  Future<void> initWithInvite(CallType type, RTCSessionDescription offer,
    ...
    // ChatFlow: presenting an incoming call must not acquire camera/mic.
    // Capture starts in answer(), after the application obtains consent.
  ...
  bool get _answerTerminated =>
      state == CallState.kEnding || state == CallState.kEnded;

  Future<void> answer({String? txid}) {
    return _answerInFlight ??=
        _answerAfterConsent(txid: txid).whenComplete(() => _answerInFlight = null);
  }

  Future<void> _answerAfterConsent({String? txid}) async {
    if (_inviteOrAnswerSent || _answerTerminated) {
      return;
    }
    if (direction == CallDirection.kIncoming &&
        !isGroupCall && localUserMediaStream == null) {
      // A permission/device failure leaves the invite actionable for retry.
      final stream = await _getUserMedia(type, endOnFailure: false);
      if (stream == null) throw StateError('Local media unavailable');
      if (_answerTerminated) {
        for (final track in stream.getTracks()) {
          await track.stop();
        }
        await stream.dispose();
  ...
  Future<MediaStream?> _getUserMedia(CallType type,
      {bool endOnFailure = true}) async {
    ...
    try {
      ...
      if (endOnFailure) await _getUserMediaFailed(e);
      rethrow;
    }
  }
