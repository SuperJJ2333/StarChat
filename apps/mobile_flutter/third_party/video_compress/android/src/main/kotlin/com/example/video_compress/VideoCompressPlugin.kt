    ...
    override fun onMethodCall(call: MethodCall, result: MethodChannel.Result) {
        ...
        when (call.method) {
            ...
            "compressVideo" -> {
                ...
                val out = UUID.randomUUID().toString()
                ...
                val videoBitrate = call.argument<Int>("videoBitrate")
                val maxDimension = call.argument<Int>("maxDimension")
                if (videoBitrate != null && maxDimension != null) {
                    videoTrackStrategy = DefaultVideoStrategy.atMost(maxDimension, maxDimension)
                ...
                audioTrackStrategy = if (includeAudio) {
                    val sampleRate = call.argument<Int>("audioSampleRate") ?: DefaultAudioStrategy.SAMPLE_RATE_AS_INPUT
                    val channels = call.argument<Int>("audioChannels") ?: DefaultAudioStrategy.CHANNELS_AS_INPUT

                    DefaultAudioStrategy.builder()
                        .channels(channels)
                        .sampleRate(sampleRate)
                        .bitRate((call.argument<Int>("audioBitrate") ?: 64000).toLong())
                        .build()
                } else {
                    RemoveTrackStrategy()
                }
