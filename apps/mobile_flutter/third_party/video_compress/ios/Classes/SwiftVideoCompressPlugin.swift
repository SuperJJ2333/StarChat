...
public class SwiftVideoCompressPlugin: NSObject, FlutterPlugin {
    ...
    private var chatEncoder: ChatVideoEncoder?
    ...
    public func handle(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
        ...
        case "compressVideo":
            if let args = args, args["videoBitrate"] is Int {
                let encoder = ChatVideoEncoder()
                chatEncoder = encoder
                encoder.start(path: args["path"] as! String, args: args,
                    progress: { value in self.channel.invokeMethod("updateProgress", arguments: value) },
                    completion: { url in
                        self.chatEncoder = nil
                        guard let url = url else { result(nil); return }
                        var json = self.getMediaInfoJson(url.path)
                        json["isCancel"] = false
