...
final class _VoiceComposerState extends State<VoiceComposer> {
  ...
  Future<void> send() async {
    final local = previewPath;
    if (local == null) return;
    controller.confirmSend();
    try {
      await widget.service.sendVoicePreview(widget.roomId, local);
      message = '语音已加密发送';
      controller.discard();
      previewPath = null;
    } catch (_) {
  ...
  Future<void> discard() async {
    final local = previewPath;
    if (local != null) {
      await File(local).delete().catchError((_) => File(local));
    }
    previewPath = null;
    controller.discard();
    if (mounted) setState(() {});
  ...
  Widget build(BuildContext context) => Padding(
      ...
      child: Column(mainAxisSize: MainAxisSize.min, children: [
        WeChatHoldToTalk(
            ...
            onStop: stop,
            onCancel: cancel),
        if (controller.state == VoiceRecordingState.preview) ...[
          const SizedBox(height: 12),
          Text('录音 ${controller.duration?.inSeconds ?? 0} 秒，确认后发送'),
          Row(mainAxisAlignment: MainAxisAlignment.center, children: [
            ...
            ModernActionButton(
                icon: CupertinoIcons.paperplane, label: '发送', onPressed: send)
          ])
        ],
