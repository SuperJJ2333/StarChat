...
final class MatrixLoginGrant { const MatrixLoginGrant({required this.loginToken,required this.homeserver,required this.expiresIn});final String loginToken;final String homeserver;final int expiresIn; }
abstract interface class DualDomainBusinessGateway { Future<void> loginBusiness({required String username,required String password,required String deviceKey,required String deviceName});Future<MatrixLoginGrant> issueMatrixLoginToken();Future<void> bindMatrixUserId(String matrixUserId);Future<void> logoutBusiness(); }
abstract interface class MatrixTokenLoginGateway { bool get isLoggedIn;String? get userId;Future<void> loginWithToken({required String loginToken,required Uri homeserver});Future<void> sync(); }
final class DualDomainLoginService {
  const DualDomainLoginService({required this.business,required this.matrix,required this.deviceKey});
  final DualDomainBusinessGateway business;final MatrixTokenLoginGateway matrix;final String Function() deviceKey;
  Future<void> login(String username,String password) async {await business.loginBusiness(username:username,password:password,deviceKey:deviceKey(),deviceName:'六合通移动端');try{if(matrix.isLoggedIn){await matrix.sync();}else{final grant=await business.issueMatrixLoginToken();await matrix.loginWithToken(loginToken:grant.loginToken,homeserver:Uri.parse(grant.homeserver));await matrix.sync();}final matrixUserId=matrix.userId;if(matrixUserId==null)throw StateError('Matrix login did not return a user id');await business.bindMatrixUserId(matrixUserId);}catch(_){await business.logoutBusiness();rethrow;}}
}
...
final class LoginController extends ChangeNotifier {
  ...
  factory LoginController.dualDomain({required DualDomainBusinessGateway business,required MatrixTokenLoginGateway matrix,required String Function() deviceKey,Future<void> Function(Duration)? delay}){final service=DualDomainLoginService(business:business,matrix:matrix,deviceKey:deviceKey);return LoginController(operation:service.login,delay:delay);}
