...
void main() {
  ...
  const description =
      'native audio and supported camera honor actual iOS grants';
  test(description, () async {
    addTearDown(binding.postTest);
    // These are native method-channel assertions, with no widget tree or
    // test-owned semantics handles. Keep the integration result/error reporter
    // without WidgetTester's unrelated live accessibility handle audit.
    await binding.runTest(() async {
      ...
      for (final permission in [
        ...
        expect(await permission.status, PermissionStatus.granted,
            reason:
                '$permission must read the pre-granted native authorization');
