...
class CallForegroundService : Service() {
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        try {
            ...
            // System foreground-service restrictions must not crash the app.
            showFallbackNotification(this, video)
            stopSelf()
            return START_NOT_STICKY
        }
        // 无应答超时：结束整通呈现（通知/铃声/CallManager 状态联动清理），
        // 不再只停服务留下悬挂的 ringing 状态。
        ringTimeout?.let { timeoutHandler.removeCallbacks(it) }
        val presentationId = CallManager.callId
        ringTimeout = Runnable {
            if (CallManager.callId == presentationId &&
                CallManager.state == CallManager.State.ringing) {
                CallManager.onEnded()
            }
        }.also { timeoutHandler.postDelayed(it, RING_TIMEOUT_MS) }
        return START_NOT_STICKY
    }
    ...
    companion object {
        ...
        fun start(context: Context, video: Boolean) {
            ...
            try {
                ...
                // 通知（无前台服务，锁屏全屏仍可用）。
                showFallbackNotification(context, video)
            }
        }

        private fun showFallbackNotification(context: Context, video: Boolean) {
            runCatching {
                val nm = context.getSystemService(Context.NOTIFICATION_SERVICE)
                    as android.app.NotificationManager
                nm.notify(CallNotificationManager.notificationId,
