...
class MomentsService:
    ...
    def _user_projection(self, session, user_id, viewer_id=None):
        ...
        if viewer_id and viewer_id != user_id:
            contact = session.scalar(select(ContactProfile).where(ContactProfile.owner_id == viewer_id, ContactProfile.contact_id == user_id))
            if contact and contact.remark and contact.remark.strip():
                remark = contact.remark.strip()
