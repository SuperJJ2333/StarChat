...
class AdjustmentWorkflow:
    ...
    def admin_review(self, request_id: str, *, reviewer_id: str, approve: bool) -> AdjustmentRequest:
        with self.session_factory() as session:
            ...
            if not request or abs(request.amount) <= self.admin_threshold:
                raise ValueError("admin approval not required")
        return self._review(request_id, reviewer_id, approve, "FINANCE_APPROVED", "ADMIN_APPROVED", "admin_reviewer_id")
    ...
    def execute(self, request_id: str, *, actor_id: str, idempotency_key: str) -> AdjustmentRequest:
        with self.session_factory() as session:
            ...
            if abs(request.amount) > self.admin_threshold and request.status != "ADMIN_APPROVED":
                raise ValueError("admin approval required")
