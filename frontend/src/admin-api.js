...
export function createAdminApi({ baseUrl = DEFAULT_BASE_URL, token = null, tokenProvider = null, fetchImpl = globalThis.fetch } = {}) {
  ...
  return {
    ...
    previewOwnerTransfer: async body=>request('/api/v1/admin/wallet/manual/owner-transfers/preview',{method:'POST',headers:{'Content-Type':'application/json'},cache:'no-store',body:JSON.stringify(body)}),
    executeOwnerTransfer: async (body,options)=>command('/api/v1/admin/wallet/manual/owner-transfers',body,options),
    getOwnerTransfer: async txid=>request(`/api/v1/admin/wallet/manual/owner-transfers/${encodeURIComponent(txid)}`,{cache:'no-store'}),
