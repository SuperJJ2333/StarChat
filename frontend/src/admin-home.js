...
function platformButtons() {
  ...
  ios.href = "/download";
  ios.setAttribute("aria-label", "下载 iOS 正式版 0.3.102（2144）");
  const iosLabel = element("span", "land-platform-chip", "iOS 版下载");
  iosLabel.append(element("span", "land-platform-status", "0.3.102（2144）· 企业正式版"));
  ios.append(iosLabel);
  actions.append(ios);
  return actions;
}
...
function homeView() {
  ...
  downloadCopy.append(downloadHead, element("p", "land-download-note", "Android 安装包由官方渠道分发；iOS 正式版 0.3.102（2144）请前往安装页，使用 Safari 安装或扫码下载。"));
