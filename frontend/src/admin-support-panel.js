...
export function supportPanel(api,{mode='both'}={}){
  ...
  if(mode!=='grant'){
    ...
    form.addEventListener('submit',async e=>{
      ...
      if(!/^[\u4e00-\u9fff]{2,6}$/.test(badge.value)){feedback.textContent='客服后缀须为2–6个汉字';return;}
      submit.disabled=true;
      try{await api.command(`/api/v1/admin/support-roles/${encodeURIComponent(target.value)}`,{role_code:role.value,badge:badge.value},{idempotencyKey:key()});feedback.textContent='已保存';await load();}
      catch(error){feedback.textContent=`保存失败：${error.message??'请重试'}`;}
  ...
  if(mode!=='manage'){
    ...
    form.addEventListener('submit',async e=>{e.preventDefault();if(submit.disabled)return;const body={user_id:target.value.trim(),amount:amount.value.trim(),reason_code:reason.value};const fingerprint=JSON.stringify(body);const commandKey=grantKeys.get(fingerprint)??key();grantKeys.set(fingerprint,commandKey);submit.disabled=target.disabled=select.disabled=amount.disabled=reason.disabled=true;try{const result=await api.command('/api/v1/admin/finance/adjustments',body,{idempotencyKey:commandKey});feedback.textContent=`已发放：${result.amount} 点钻`;grantKeys.delete(fingerprint);target.value='';amount.value='';}catch(error){feedback.textContent=`发放失败：${error.message??'请重试'}`;}finally{submit.disabled=target.disabled=select.disabled=amount.disabled=reason.disabled=false;}});panel.append(form);
  }
