...
export function walletRepairDialog(api,item,{actorId,storage=globalThis.localStorage,onClose=()=>{},onCompleted=()=>{}}={}){
  ...
  function fields(target,value){
    ...
    for(const [key,label] of [['intent_id','充值订单号'],['order_id','提现订单号'],['username','畅聊号'],['nickname','用户名'],['user_id','用户编号'],['amount','链上金额'],['expected_amount','订单金额'],['asset','币种'],['network','链类型'],['official_address','官方地址'],['source_address','付款地址'],['target_address','目标地址'],['log_index','日志索引'],['order_status','提现订单状态'],['binding_id','地址绑定编号'],['binding_version','绑定版本'],['effective_from_block','绑定生效区块'],['effective_to_block','绑定失效区块'],['expires_at','订单有效期'],['intent_status','充值订单状态'],['status','处理状态'],['payout_status','提现核对状态'],['review_reason','核对结果说明'],['receipt_status','收款状态'],['ledger_transaction_id','账本交易编号'],['txid','交易哈希'],['block_time','链上时间'],['created_at','订单创建时间'],['payment_before_order_seconds','付款早于建单（秒）']])if(value[key]!==undefined&&value[key]!==null)dl.append(node('dt',label),node('dd',key.endsWith('_at')||key==='block_time'?formatBeijingTime(value[key]):value[key]));target.append(dl);
  }
  function completed(result){
    if(result.status!=='EXECUTED')return;
    try{storage.removeItem(journalKey);}catch{}
    retryOperation=undefined;
    Promise.resolve().then(()=>onCompleted(result)).then(refreshed=>{
      if(!disposed&&refreshed===false)status.textContent=inflow?'已入账；链上列表刷新失败，请手动刷新。':'提现核对已执行；链上列表刷新失败，请手动刷新。';
    }).catch(()=>{
      if(!disposed)status.textContent=inflow?'已入账；链上列表刷新失败，请手动刷新。':'提现核对已执行；链上列表刷新失败，请手动刷新。';
    });
  }
  async function lookup(operation){
    status.textContent='只查询请求结果，不重新提交…';try{const result=await api.getWalletRepair(kind,operation);if(disposed)return;status.textContent=`当前结果：${result.status??'已查询'}；请核对账本或提现订单的最终状态。`;body.replaceChildren();fields(body,result);completed(result);}catch(error){if(!disposed)status.textContent=`结果暂未确认：${error.message}。保留请求编号 ${operation}，请稍后再次查询。`;if(!disposed&&error.status===404){retryOperation=operation;body.append(button('重新预检（保留原请求编号）',()=>void start()));}}
  }
  function confirm(value){
    preview=value;body.replaceChildren();body.append(steps());
    actionSummary(value.confirmation??{});if(value.blockers?.length){const reason=value.blockers.map(repairBlocker).join('；');status.textContent=reason;disabledConfirm(reason);body.append(button('返回修改',()=>void start()));manualEntry();evidence(value.confirmation??{});return;}
    status.textContent=`预检通过后请勾选确认并提交；${inflow?'本次预检不入账':'本次预检不提交提现核对'}，有效至 ${formatBeijingTime(value.expires_at)}。提交时服务器会再次核验，操作不会自动重放。`;
    ...
    const execute=button(inflow?'确认补入账':'确认提交（提现核对）',async()=>{
      ...
      try{const result=await api.executeWalletRepair(kind,{preview_id:preview.preview_id,digest:preview.digest,expected_version:preview.expected_version,operation_id:operation,confirmed:true},{idempotencyKey:operation});if(disposed)return;status.textContent=result.status==='EXECUTED'?(inflow?'已入账，正在刷新链上列表。':'提现核对已执行；账本交易编号如下。'):`提交结果：${result.status??'已接收'}。请刷新当前操作状态核对最终结果。`;body.replaceChildren();actionSummary(result);fields(body,result);completed(result);}
    ...
    });execute.className='admin-primary';execute.disabled=true;check.addEventListener('change',()=>{execute.disabled=!check.checked||submitted;});body.append(label,execute,button('返回修改',()=>{if(!submitted)void start();}));manualEntry();evidence(value.confirmation??{});
  }
  function form(){
    const form=node('form',undefined,'admin-command-form admin-repair-form');
    ...
    const reason=node('select',undefined,'admin-filter');reason.setAttribute('aria-label','异常原因');for(const [value,label] of Object.entries(reasons)){const option=node('option',label);option.value=value;reason.append(option);}
  ...
  async function start(query=''){
    ...
    try{const result=await api.getDepositRepairCandidates({txid:item.txid,log_index:String(item.log_index),query});if(disposed||version!==revision)return;receiptId=result.receipt_id;
      ...
      for(const candidate of result.items??[]){const row=node('tr');for(const value of [candidate.intent_id,`${candidate.username??'—'} / ${candidate.nickname??'—'}`,`${candidate.expected_amount} USDT`,candidate.network,candidate.intent_status])row.append(node('td',value));const cell=node('td');cell.append(button('选择',()=>{selected=candidate;body.replaceChildren();form();evidence(candidate);}));row.append(cell);tbody.append(row);}table.append(thead,tbody);const scroll=node('div',undefined,'admin-table-scroll');scroll.append(table);body.append(steps(),scroll);
      ...
      disabledConfirm('请先选择订单并完成预检；预检不会入账。');manualEntry();
