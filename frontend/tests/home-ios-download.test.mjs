...
test('download page advertises the current IPA and retains OTA installation', () => {
  ...
  assert.match(source, /itms-services:\/\/\?action=download-manifest&(amp;)?url=https:\/\/www\.liuhetong888\.com\/downloads\/ios\/manifest\.plist"/);
  assert.doesNotMatch(source, /manifest\.plist\?/);
