# 畅聊 ChatFlow Matrix/Element Private Messaging Stack
...
The repository is being upgraded from the original integration skeleton into the approved 畅聊 ChatFlow product. The authoritative design and delivery roadmap are:
...
Phase 0 repairs configuration rendering, locks deployment inputs, establishes tests and Agent rules, and restricts the existing Bot to explicitly authorized notification rooms. Flutter, the business backend, 点钻 ledger/red packets, USDT custody integration, encrypted calls, and the React administration application are delivered by later phases and must not be inferred from the current skeleton.
