...
## 2026-09-25 新手机号缺邀请码时的已验证续行（用户明确要求；领域与安全复审通过）

用户要求：新手机号短信验证通过后，即使尚未填写邀请码，也能补填并继续注册，无需再次取码。生产阿里云 `CheckSmsVerifyCode` 未承诺相同验证码在第一次 PASS 后可再次通过；仅回滚数据库 `consumed_at` 不能回滚供应商状态。因此不得通过客户端保留六位码或自行标记“已验证”实现续行。

对声明支持该能力的新客户端，首次 `/auth/phone/login` 供应商验证 PASS 且确认本地尚无账号时，在**同一事务**消费 OTP 并创建 5 分钟的 `login_invitation` 证明；先返回 `INVITATION_VERIFIED`，不消耗邀请码、不创建账号或 Outbox。证明使用 256 位随机 bearer ticket，数据库只存 HMAC 摘要，并绑定已验证手机号摘要与设备键摘要。旧客户端默认行为保持，必须由新客户端显式 opt in。

`/auth/phone/login/invitation` 只在持有该 ticket、匹配手机号/设备且证明未过期时检查用户协议和邀请码。无效、过期、耗尽邀请码返回封闭纠正状态；有界次数内可换邀请码重试，不再调用短信供应商。成功时在单个数据库事务内消耗邀请码、创建用户和 Matrix provisioning Outbox，并把同一证明变为既有 `login_resume` 票据。邀请提交响应丢失时，用同一票据继续查询已提交的 Matrix 状态，不重复注册。票据不可用于另一手机号、设备或已经存在的账号；次数用尽、过期、撤销或成功消费后拒绝重放。

客户端仅在**收到服务端明确的已验证 ticket**时显示“验证码已通过”并以内存保存 ticket；未知网络结果、错码及旧 Debug2175 的 422 不产生该状态。更换手机号或新发验证码时清除本地续行状态。服务端不持久化或记录验证码、ticket 原文、邀请码原文、手机号明文日志、Matrix/E2EE 密钥。外部供应商 PASS 与数据库提交无法原子化，若恰在二者之间崩溃，本次 ticket 不会签发，必须重新取码；此限制不能隐藏。
