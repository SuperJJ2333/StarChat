...
- 关联计划：原工作区 2026-09-24 已批准的公告/钱包/客服计划第 2 项及其验收记录；该历史计划尚未检入本分支。隔离分支从 `9cfcd00c` 建立，修复提交已重放至 `main` 的 `2442f0ab` 之上；生产发布以当次运行镜像为基底，仅覆盖两文件。
- 当前状态：完成；全仓门禁、API-only 生产发布、机器验收及 Android/iOS 用户侧复验均通过。
...
- 最后更新：2026-09-25 18:22 HKT。
- 下一步：观察账单查询延迟 P95；本次不调整财务读架构。

## 验收台账

| ID | 场景及预期 | 实现 | 测试及证据 | 发布 | 真机反馈/缺口 |
| --- | --- | --- | --- | --- | --- |
| BILL-01 | 账单列表/详情展示两位小数的逐笔点钻余额 | `StatementService` 按用户 CAIBI 流水的 `(created_at,id)` 顺序累计，`StatementItem.balance_after` 返回字符串 | 首个测试 red：`KeyError: balance_after`；green：账单测试 18/18，OpenAPI check 退出 0 | API-only 已发布；运行时 OpenAPI 字段存在 | 用户 2026-09-25 回复：Android、iOS 下拉刷新后均显示数字 |
...
| 双端真机回馈 | 2026-09-25 18:22 左右 | 用户确认 Android 和 iOS 的 V0.4.7 刷新后点钻余额都显示数字；不收集账单明细或账号 |
...
- 发布状态：候选镜像 `2547aafd…` 已作为生产 API 单服务发布，运行冻结配置 `/opt/starchat/releases/guarded-3v1b02yk/compose.json`。用户确认两端账单刷新后显示数字；没有收集生产认证响应中的用户明细。候选/回退配置及数据库备份在服务器 0700 目录 `/opt/starchat/releases/bill-balance-20260925-01`，回退目标为切换前 `25954c6a…` 镜像；旧 `0087` 发布脚本不适用于当前 `0088`。独立测试 PG 容器已按其 ID/网络隔离核对后清理。源码提交已推送远端 `main`，本地 `main` 快进到同一提交且保留原有 67 项未提交改动。
