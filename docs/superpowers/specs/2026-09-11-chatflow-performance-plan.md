...
以下“代码已确认”表示静态源码存在对应路径；“待采样”表示尚未证明它是该设备症状的直接触发点。所有耗时、内存及掉帧数字均为建议验收目标，不是实测结果，也不是 Telegram 官方性能数据。制定本方案时仅授权调查；后续用户已授权按执行计划修改本地源码并做自动化验证，真机验收由用户负责，未授权自动部署或生产操作。
