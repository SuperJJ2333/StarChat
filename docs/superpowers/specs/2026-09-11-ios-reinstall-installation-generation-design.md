...
探测失败（目录不可读等）按"不确定"处理：既不删除也不写标记，并由启动门禁阻止业务与
Matrix 初始化、显示重试入口；与标记读取失败同一条保守规则。
...
1. **标记读取失败 → 既不清除也不写标记，也不构造业务 API、Matrix 工厂或数据库。**
   `InstallationStartupGate` 显示“启动检查未完成，请重试”；仅在用户重试成功后单次启动。
2. **容器探测失败 → 同样既不清除也不写标记，且阻止启动。** 探测回答的是“密钥是否还有效”，
...
- 不修改 `continuityMetadata` 的既有空库兼容路径：SDK 完全没有 session/user/device 时，
  它只清除失效 binding 并采用空库；真实 identity/fingerprint 不匹配仍然失败关闭。
  `InstallationReconciler` 在标记或探测不确定时仍不清理安装世代密钥、注册表或业务会话。
...
- **启动核对失败不会固化错误判定。** `InstallationStartupGate` 在任何 `failed` 结果前
  阻止 `BusinessApiClient`、`MatrixClientFactory` 和数据库构造，因此不会由同一次失败
  创建库文件并在下一次误判为覆盖升级。它不写 pending 标记：失败后若用户有新的有效登录，
  延迟清理可能删除新密钥；唯一允许的恢复动作是本次错误页上的显式重试。
