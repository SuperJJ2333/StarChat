...
> **2026-09-21 修订（用户需求书批准，替代本文冲突条款）：** ① 注册：在邮箱之外新增**中国大陆手机号注册与短信验证码登录**（非实名，ADR-0075）——§1.1、§2"不做手机号注册"、§6.1、§18.1 中"不要求手机号/无手机号字段"相应失效；② 计价：点钻按 **1 点钻 = 1 元人民币**计价（ADR-0076），§9.1"不允许兑换"由 ADR-0010 引入的用户兑换已被 ADR-0076 关闭；③ 充值/提现改为**人工客服结算**（ADR-0077），§12 在线自动充值默认关闭；④ 红包：0.5% 手续费（ADR-0073）+ 群主抽成 0.1% 与满 10 人群主免手续费（ADR-0078）——§11.1"不收手续费"失效；⑤ 群主转让冷却（ADR-0079）。其余条款继续有效。
