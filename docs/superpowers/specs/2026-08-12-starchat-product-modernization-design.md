...
**状态：** 已批准  
**日期：** 2026-08-12  
**适用仓库：** `StarChat`（产品展示名：六合通）  
**目标基线：** 10 万注册用户、1 万日活、2,000 同时在线、1000 人群组、每日 10 万笔积分/红包交易、账本峰值 50 TPS、单区域部署

> **2026-09-03 修订（展示名统一）：** 用户可见的产品与资产展示名以根目录 `CONTEXT.md` 词汇表为唯一准绳——产品名「畅聊 ChatFlow」（避免「六合通」）、资产展示名「点钻」（避免「彩币」）。本文正文中出现的「六合通」「彩币」均为历史用名，仅作背景保留。内部标识（仓库名 StarChat、内部项目标识 liuhetong、Matrix server name、内部资产代码 CAIBI、Android 包名等）保持稳定不变，不随展示名改名。

> **2026-09-09 修订：** 用户批准千人群容量目标及共享 media_id＋独立引用、可信内容哈希缓存方案。附件确定性加密的参数、已接受风险与兼容要求见 [ADR-0060](../../adr/0060-content-addressed-media-dedup.md) 和 [实施计划](../plans/2026-09-09-content-addressed-media-thousand-member.md)。关闭 presence，保持 Megolm 默认轮换；本修订不代表千人容量已经实测通过。
...
- 彩币展示名：`彩币`
- 彩币内部资产代码：`CAIBI`
- 旧仓库名、目录名和 Matrix server name 保持配置化，不通过改名破坏既有 Matrix ID。

## 1. 目标

将当前 Matrix Synapse、Element Web 与 FastAPI Bot 集成骨架改造为完整的六合通产品，交付：
...
1. 校验注册邀请码，包括有效期、剩余次数和允许用户类型。
2. 接收唯一用户名、唯一邮箱和密码。
3. 发送一次性邮箱验证码并执行速率限制。
4. 验证成功后，在一个业务事务中创建用户、零余额 CAIBI/USDT 账户和 Matrix 映射记录。
...
### 6.2 邀请码（2026-09-03 修订：统一为单一注册邀请码体系）

全流程只有一套**注册邀请码**，不再存在独立的"好友邀请码"：

- 管理员可批量签发：有效期、最大使用次数、用户类型。
- 每个活跃用户拥有一个**固定的个人注册邀请码**（`GET /api/v1/invitations/mine` 首次读取时派生，HMAC 确定性生成、不轮换；默认最大使用次数 20、有效期滚动 365 天），"我"页展示与分享该码。
- 邀请关系由注册时**消耗的邀请码归属**推导：注册消耗了某用户的个人邀请码即与该用户建立邀请关系（复用 `referral_bindings` 记录）；旧客户端提交的 `referral_code` 参数在过渡期继续受理。
- 邀请关系不形成默认层级返佣关系。
- 注册消耗邀请码必须与用户创建原子提交。
...
- 用户可用余额不得为负。
- CAIBI 精度为2位，USDT 精度为6位。
- 不允许跨资产平衡，不允许 CAIBI/USDT 兑换。
- 纠错只能创建关联原交易的反向冲正交易。
...
- 账本峰值50 TPS。
- 1000人群组同步（容量目标；全量验收待新集群压测，不以本地 200–500 VU 代替）。
