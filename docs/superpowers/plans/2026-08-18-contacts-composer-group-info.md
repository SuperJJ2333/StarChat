...
- [ ] 写 393 × 852 overscroll Widget 红灯测试，断言导航标题与联系人内容共同下移且底部导航不移动。
- [ ] 运行 focused test，确认因现有固定 `navigationBar` 结构失败。
- [ ] 将通讯录改为同一可回弹滚动表面并保持索引定位、分组和 TabBar 约束。
- [ ] 运行 focused test 并格式化。
...
- [ ] 写无焦点 Emoji 也显示发送及发送容器 pressed 色的红灯测试。
- [ ] 运行测试，确认现有 `focused && hasText` 和透明按钮导致失败。
- [ ] 改为 `hasText` 决定发送态，并给发送按钮增加 `brand/pressed` 背景和对比前景。
- [ ] 运行 focused test 并格式化。
...
- [ ] 写少于两名受邀者拒绝、默认前三成员名、20 字截断的红灯测试。
- [ ] 运行测试并确认失败原因来自旧规则。
- [ ] 实施总人数至少三人、稳定成员顺序和字符级 20 字限制。
- [ ] 运行 focused test 并格式化。
...
- [ ] 写 controller 红灯测试，覆盖成员计数、折叠数量、名称/公告/备注、添加成员与退出群聊。
- [ ] 写页面红灯测试，覆盖 `聊天信息(n)`、末位添加图标和功能顺序。
- [ ] 实施 Matrix gateway 与账号房间设置，接入群聊 Chat 右上角更多入口。
- [ ] 实施微信式分组页面、成员选择、编辑页与错误反馈。
- [ ] 运行 focused tests 并格式化。
...
- [ ] 在 `20 Messages & Chat` 区域创建七个独立 393 × 852 Frame。
- [ ] 使用现有 Token、真实矢量图标和严格层级构建群聊 Chat、详情、展开、选择、编辑与搜索状态。
- [ ] 对每个 Frame 获取截图，校验字体、颜色、尺寸、裁切和顺序。
...
- [ ] 运行 focused tests、`flutter test` 和 `flutter analyze`。
- [ ] 运行 `pwsh -NoProfile -File scripts/verify.ps1`。
- [ ] 构建 Debug APK，安装至在线雷电模拟器并截图验证三条流程。
- [ ] 记录红绿灯、Figma node IDs、测试、APK 和 ADB 证据，完成规格与质量复审。
