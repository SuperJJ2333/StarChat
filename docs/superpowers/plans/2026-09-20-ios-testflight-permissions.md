...
- [ ] 权限TDD：真实平台通道状态与请求结果回归，Pod配置及原生二进制能力门禁；实现最小修复。
- [ ] TestFlight：修正并复用已成功的ios-0353签名步骤，提前检查profile与ASC访问/版本；仅测试分发，无企业产物任务。
- [ ] 专项、analyze、Flutter全量及适用verify（同源未改后端证据可复用）；规格后质量安全审查。
