...
- [x] V1/D1：Astra亲审实际diff/调用链及测试证据，定向→完整Flutter/analyze/UI契约/前端/verify，如实对比既有失败。核对版本占用后源码构建→APK重建→固定签名→语义/哈希核验，Mi6 install-r保留数据；用户功能自测。不push/部署/迁移、不更改设备WiFi设置。
