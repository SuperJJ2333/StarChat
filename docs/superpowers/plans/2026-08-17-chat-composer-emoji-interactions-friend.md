...
- [x] **Step 1: 写状态机和 Widget 失败测试**，断言 DOM 顺序为 voice → input → emoji → more/send，输入聚焦且有文字时 `composer-more` 消失、`composer-send` 出现，并断言四个触控目标至少 44 px。
- [x] **Step 2: 运行红灯**：`flutter test test/ui/chat_composer_bar_test.dart`，预期因 `chat_composer_state.dart` 不存在及旧附件顺序失败。
- [x] **Step 3: 最小实现状态模型与输入栏**；使用 `FocusNode` 和 controller listener 刷新状态，输入框占据剩余宽度，more/send 共用末端位置。
- [x] **Step 4: 接入 `RoomPage`**，把旧 `onAttachment` 改为 `onMore`，文本发送后保留焦点并清理输入。
- [x] **Step 5: 运行绿灯**：`flutter test test/ui/chat_composer_bar_test.dart test/ui/wechat_components_test.dart`。
- [x] **Step 6: 提交**：`git commit -m "feat(mobile): align chat composer with figma"`。
...
- [x] **Step 1: 写失败测试**，断言 4 列网格完整显示六项真实 Icon，表情页存在“最近/全部/我的表情”，GIF 使用保持动画的 `Image`。
- [x] **Step 2: 运行红灯**：`flutter test test/ui/chat_more_panel_test.dart test/ui/chat_emoji_panel_test.dart`。
- [x] **Step 3: 固定兼容版本的 `emoji_picker_flutter` 并执行 `flutter pub get`**；不得把 Unicode 表情列表写回页面代码。
- [x] **Step 4: 实现两个面板并替换 `_showMedia`/`_showEmoji` 弹窗**；图片、拍摄、语音、视频、红包、文件调用现有公开回调。
- [x] **Step 5: 运行绿灯与 analyze**：`flutter test test/ui/chat_more_panel_test.dart test/ui/chat_emoji_panel_test.dart && flutter analyze`。
- [x] **Step 6: 提交**：`git commit -m "feat(mobile): add chat more and emoji panels"`。
...
- [x] **Step 1: 写失败测试**，覆盖 SHA-256 去重、乱序事件合并、删除 tombstone、最近使用稳定合并，以及拒绝未加密房间。
- [x] **Step 2: 运行红灯**：`flutter test test/features/matrix/emoji_vault_test.dart`。
- [x] **Step 3: 实现纯 Dart 合并模型**，不依赖 Widget 或业务 API。
- [x] **Step 4: 实现 Matrix 适配器**，复用现有加密上传能力；账号数据只保存仓库 room ID，不保存表情明文或密钥。
- [x] **Step 5: 接入图片/GIF“添加到表情”与我的表情页**，同步错误显示可重试状态。
- [x] **Step 6: 运行绿灯并提交**：`flutter test test/features/matrix/emoji_vault_test.dart`；`git commit -m "feat(mobile): sync encrypted custom emoji vault"`。
...
- [x] **Step 1: 写失败测试**，逐类型断言菜单能力，特别断言文本消息没有“添加到表情”、图片/GIF有；删除后当前设备过滤事件且 fake Matrix client 零调用。
- [x] **Step 2: 运行红灯**：运行上述两个测试文件。
- [x] **Step 3: 实现能力策略、长按菜单和本地隐藏存储**；存储键按账号和 room ID 隔离。
- [x] **Step 4: 接入多选模式、批量转发和批量本地删除**；红包和系统控制事件禁止转发。
- [x] **Step 5: 运行绿灯并提交**：`git commit -m "feat(mobile): add typed message actions and local delete"`。
...
- [x] **Step 1: 写失败测试**，覆盖 1:59 可撤回、2:01 不可撤回、他人消息不可撤回、Redaction 只调用一次、reply relation 和 `m.mentions.user_ids`。
- [x] **Step 2: 运行红灯**：`flutter test test/features/matrix/message_interaction_service_test.dart`。
- [x] **Step 3: 实现最小服务并接入菜单**；转发时在目标房间重新加密，严禁复制密钥或原始密文。
- [x] **Step 4: 长按头像插入 `@显示名` 并保存 Matrix 用户 ID**；取消/编辑提及后同步更新 ID 集合。
- [x] **Step 5: 运行绿灯并提交**：`git commit -m "feat(mobile): add reply forward recall and mentions"`。
...
- [x] **Step 1: 写失败测试**，双击头像只发送一次 nudge；提醒事件加密同步、同 ID 幂等、取消通知、离线过期后提示。
- [x] **Step 2: 运行红灯**：运行两个服务测试。
- [x] **Step 3: 固定兼容版本 `flutter_local_notifications` 并实现本地 scheduler**；通知正文只使用通用文案。
- [x] **Step 4: 实现加密提醒控制房间合并和拍一拍后缀配置同步**。
- [x] **Step 5: 接入双击头像、提醒时间选择器和时间线系统气泡**。
- [x] **Step 6: 运行绿灯并提交**：`git commit -m "feat(mobile): sync nudges and message reminders"`。
...
- [x] **Step 1: 写失败 Widget 测试**，断言标题、列表顺序、真实 Icon、开关语义、删除二次确认和回调导航。
- [x] **Step 2: 运行红灯**：`flutter test test/features/contacts/contact_flow_test.dart`，预期旧“更多”和纵向大按钮结构失败。
- [x] **Step 3: 以 `CupertinoListSection` 和小型专用 section widget 最小重构两个页面**；保持现有 `ContactsGateway` 调用不变。
- [x] **Step 4: 运行 393 × 852 浅色/深色 Widget 测试**，修复任何 overflow。
- [x] **Step 5: 运行绿灯并提交**：`git commit -m "feat(mobile): map friend screens to figma"`。
...
- [x] **Step 1: 运行格式化、analyze、全部 Flutter 测试**：`dart format --output=none --set-exit-if-changed lib test`; `flutter analyze`; `flutter test`。
- [x] **Step 2: 运行仓库验证**：仓库存在时执行 `pwsh -NoProfile -File scripts/verify.ps1`，完整保存成功或失败输出摘要。
- [x] **Step 3: 构建 APK 并计算 SHA-256**：`flutter build apk --debug`; `Get-FileHash build/app/outputs/flutter-apk/app-debug.apk -Algorithm SHA256`。
- [x] **Step 4: 通过 `adb devices` 识别雷电模拟器，安装 `adb install -r`，启动畅聊并执行登录—消息—好友资料—表情—主题冒烟流程。
- [x] **Step 5: 写验证证据并勾选本计划已完成任务**，不得把 token、密码、Matrix 密钥或聊天明文写入证据。
- [x] **Step 6: 最终提交**：`git commit -m "test(mobile): verify chat interaction delivery"`。
