> **Historical / 已由新入口替代 · 2026-09-20 文档整理。** 原路径 `docs/runbooks/app-release-deployment.md`。原有日期、版本及验收结论仅代表当时记录；当前入口：[操作索引](../../../runbooks/release-metadata.md)。本文不作为新的部署或资金操作授权。

# Runbook: Mobile app release deployment (Android APK / iOS enterprise OTA)

> **2026-09-20 用户批准的现行覆盖规则：** 发布改用[轻量发布门禁](../../../runbooks/release-metadata.md)；取消发布阶段的完整安装包回拉下载与重复验包。下文历史完整下载/回拉验证要求由此替代。保留构建阶段校验、上传完整性、元数据解析、平台隔离、审计及回退；最终签名依据交接确认，不能声称已重新验签。
...
Cross-session entry: [mobile delivery workflow](../../../runbooks/mobile-delivery-workflow.md) and [last observed state](../../../workflow/current-state.md). Re-read live state before publication; historical package and image values below are not current-state guarantees.
...
Use the persistent workflow in [admin-production-workflow.md](../../../runbooks/admin-production-workflow.md). The default SSH route is `ssh -J jumper -p 23421 root@207.56.8.8`; SCP uses `-J jumper -P 23421`. Prefer `scripts/starchat-server.ps1`, which enforces the configured jumper, UTF-8 and connection checks. Do not first cycle through the workstation proxy route or introduce third-party proxy subscriptions.
...
[matrix-framework-deployment.md](../../../runbooks/matrix-framework-deployment.md). Publishing this
...
[wallet-reserve-resampling-deployment.md](../../../runbooks/wallet-reserve-resampling-deployment.md).
...
[admin-readability-deployment.md](../../../runbooks/admin-readability-deployment.md).
...
[wallet-access-deployment.md](../../../runbooks/wallet-access-deployment.md). This changes the wallet
