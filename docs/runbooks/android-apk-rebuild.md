...
> **2026-09-20 用户批准的现行覆盖规则：** 发布改用[轻量发布门禁](release-metadata.md)；取消发布阶段的完整安装包回拉下载与重复验包。本页只保留构建阶段要求。保留构建阶段校验、上传完整性、元数据解析、平台隔离、审计及回退；最终签名依据交接确认，不能声称已重新验签。
...
7. 保存最终 APK、SHA256、证书指纹、工具版本、构建参数和验证记录。上传/更新弹窗必须指向验证过的最终重建 APK。发布阶段使用HEAD与元数据门禁，不再下载安装包重复验包；不能误发布原始中间包。
...
线上采用不可变的版本文件名，上传保留本地/服务器完整性控制；随后按轻量发布门禁检查HEAD和元数据，再在同一数据库事务内更新本平台设置及审计。latest-arm64.apk 符号链接独立原子切换，不能将它与数据库更新视为同一事务。保留旧文件以支持回退，不覆盖已经被客户端缓存的同名版本 APK。不同签名的 debug 安装不能直接覆盖正式包，也不能通过 install -d 绕过签名检查。
