...
- **当前 SDK：** `apps/mobile_flutter/pubspec.lock` 固定 `matrix 0.34.0`。该版本提供 `Encryption.bootstrap()`、SSSS、`m.megolm_backup.v1` room-key backup API、`m.secret.request`/`m.secret.send`、Olm 加密发送以及 `DeviceKeys.verified`/`blocked` 状态；这些是可用的实施原语，不等于应用层校验已经完成。源码核查确认 SSSS `request()` 只在发送时过滤设备，`SecretSend` 接收侧匹配的是请求时保存的旧设备集合并可直接写 cache，因此 backup-secret 路径必须由应用 adapter 在缓存前实时重查当前信任状态。
...
| blocked device | 已封禁但仍可尝试发送 to-device 事件的旧设备 | 应用 trusted-recovery adapter 自管 request ID 和单一目标设备，不调用 SDK SSSS `request()`、不注册 `pendingShareRequests`、不接受 SDK 自动 cache。收到 Olm 解密响应后、写 cache 前，从当前 `userDeviceKeys` 实时重查同 MXID、device ID、Curve25519 key、`verified == true`、`blocked == false`，再校验时限、请求和备份绑定；任一不符即丢弃且无恢复副作用。 | Task 6 计划测试先发送请求，再把目标设备标记 blocked，最后注入其他字段均有效的响应，断言响应被拒、SSSS cache 未写入且恢复未启动；Task 10 用真实 Synapse 场景复核。 | 正式规格第 5.4、9、11.3、11.5 节；SDK 0.34.0 `encryption/ssss.dart` 发送时筛选及接收后自动缓存行为；实施计划 Task 6。 | **PASS** |
