...
## “一直暂无在线记录”排查与修复（2091，`f61b734c`）

用户反馈 + 复现：好友资料页状态恒为“暂无在线记录”。排查结论（数据/服务端
均正常，为客户端缓存缺陷）：

1. 埋点存在且新鲜：`POST /auth/heartbeat`（App 前台心跳）写
   `identity_devices.last_seen_at`；生产 291/291 台设备全部有值，最新到
   分钟级。后台“在线人数”与本功能**同源**（admin.py:357 以
   `last_seen_at >= now-5min` 统计），并非另一套数据。
2. 根因：`ProfileRepository._contactsEqual` 判等未包含新字段
   `last_seen_at`——通讯录页打开时会静默刷新 `/friends`，但“其余字段未变”
   被判为相等，新值（含在线时间与好友增删）永远不写入缓存/UI；
   已删除好友（小彭）残留、真好友缺失、状态恒空一次全解释。
3. 修复：判等加入 `lastSeenAt`（红/绿测试
   `quiet refresh applies a changed last_seen_at`）；仓库套件 22 项、
   contacts+profile 共 131 项全绿。
4. 2091 包（27,305 类零变更、固定签名）04:10 安装 Mi 6。实机验证：
   通讯录变为真实 4 位好友（feng567/Beya20260908/a12345678912/
   a1014826460），已删好友消失；feng567 资料页“刚刚在线”（其设备
   19:48Z 心跳）、Beya 资料页“3小时前在线”（16:46Z）
   （fix2091-contacts/friend1/friend2.png）。
