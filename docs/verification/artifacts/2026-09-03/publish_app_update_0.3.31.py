...
用法：
  docker exec -i starchat-business-api-1 python - < publish_app_update_0.3.31.py
"""
import json
import os
import sys
...
IDEMPOTENCY_KEY = "app-update-publish-0.3.31-20260903"
APK_URL = f"https://www.liuhetong888.com/downloads/ChatFlow-{RELEASE_VERSION}-arm64.apk"
NOTES = (
    "0.3.31 更新：修复锁屏/后台一段时间后收不到消息通知的问题——"
    "新增同步连接自动检测与自愈机制，消息通道意外中断时会在一分钟内"
    "自动恢复并补拉漏掉的消息，不再需要打开应用才能收到提醒。"
)
