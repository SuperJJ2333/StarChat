...
内容：好友私聊房间统一（双方同一会话）、好友资料页"发消息"修复、
群聊非好友成员可进资料页添加、后台/锁屏通知与来电铃声恢复。

用法：
  docker exec -i starchat-business-api-1 python - < publish_app_update_0.3.29.py
"""
import json
import os
import sys
...
IDEMPOTENCY_KEY = "app-update-publish-0.3.29-20260903"
APK_URL = f"https://www.liuhetong888.com/downloads/ChatFlow-{RELEASE_VERSION}-arm64.apk"
NOTES = (
    "0.3.29 修复更新：好友私聊统一同一会话房间——无论哪一方发起聊天，"
    "双方进入同一个会话，不再出现两边各聊各的；修复好友资料页点击"
    "「发消息」提示\"无法打开加密会话\"的问题；群聊信息页中非好友"
    "成员现在可以点击头像查看资料并发起\"添加到通讯录\"；修复应用"
    "退到后台或锁屏后收不到消息通知、提示音与来电铃声的问题"
    "（新增消息同步保活服务；小米/红米设备建议在系统设置中允许"
    "畅聊\"自启动\"并将省电策略设为\"无限制\"以获得最佳效果）。"
)
