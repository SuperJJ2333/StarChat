"""发布 0.3.27+30 应用更新设置（latest_build 采用 arm64 清单值 2000+build，
兼容旧客户端构建号比较）（在 starchat-business-api-1 容器内执行）。

用法：
  docker exec -i starchat-business-api-1 python - < publish_app_update_0.3.27.py

依赖容器内已注入的 BUSINESS_DATABASE_URL / BUSINESS_JWT_SECRET。
通过管理端 RBAC 接口写入 app_settings，并以 /api/v1/app-updates/latest
（登录态）做端到端确认。
"""
import json
import os
...
IDEMPOTENCY_KEY = "app-update-publish-0.3.27-20260903"
APK_URL = f"https://www.liuhetong888.com/downloads/ChatFlow-{RELEASE_VERSION}-arm64.apk"
NOTES = (
    "0.3.27 版本更新：全新通知系统——新消息顶部横幅、专属提示音与震动，"
    "@我与特别关注强提醒，支持勿扰时段和锁屏通知隐私设置，语音视频来电"
    "铃声全面升级；邀请码统一：注册只需填写一个邀请码，「我」页的邀请码"
    "长期固定有效，好友注册填写即建立邀请关系；修复自己发送消息后消息"
    "列表出现红点的问题；桌面图标角标显示未读总数。"
)
