...
void main() {
  testWidgets('audit: resume ringing must restore incoming page', (tester) async {
    var resumed = false;
    final navigatorKey = GlobalKey<NavigatorState>();
    final notifications = _RecordingCallNotifications();
    final manager = CallUiManager(navigatorKey: navigatorKey,
      notifications: notifications, isAppResumed: () => resumed);
    final backend = _FakeCallBackend();
    final controller = CallController(backend: backend,
      permissions: _AllowedPermissions(), now: () => tester.binding.clock.now());
    manager.attach(controller);
    await tester.pumpWidget(CupertinoApp(navigatorKey: navigatorKey,
    ...
    await tester.pumpAndSettle();
    final restored = manager.isIncomingPageOpen;
    await _teardown(tester, backend.events, manager);
    controller.dispose();
    await backend.events.close();
    expect(restored, isTrue, reason: 'Resuming while still ringing must open the call page before dismissing notification');
  });
