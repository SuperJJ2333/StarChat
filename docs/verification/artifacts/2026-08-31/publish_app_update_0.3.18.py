"""发布 0.3.18+21 应用更新设置（在 starchat-business-api-1 容器内执行）。

用法：
  docker exec -i starchat-business-api-1 python - < publish_app_update_0.3.18.py
...
IDEMPOTENCY_KEY = "app-update-publish-0.3.18-20260831"
APK_URL = f"https://www.liuhetong888.com/downloads/ChatFlow-{RELEASE_VERSION}-arm64.apk"
NOTES = (
    "0.3.18 版本更新：图片选择页支持相册分类切换（最近图片/本地视频/"
    "Camera/Screenshots，最新内容排最前）与视频发送（默认压缩，勾选原图时"
    "单个视频不超过 20MB）；语音消息播放新增 QQ 式进度扫过动画，播放结束"
    "自动复位；修复红包领取详情中好友用户名与自定义头像不显示的问题，"
    "展示优先级为备注（仅自己可见）>昵称>用户名。"
)
