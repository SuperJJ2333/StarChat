"""发布 0.3.8+11 应用更新设置（在 starchat-business-api-1 容器内执行）。

用法：
  docker exec -i starchat-business-api-1 python - < publish_app_update_0.3.7.py
"""
import json
import os
import sys
...
IDEMPOTENCY_KEY = "app-update-publish-0.3.16-20260831"
APK_URL = f"https://www.liuhetong888.com/downloads/ChatFlow-{RELEASE_VERSION}-arm64.apk"
NOTES = (
    "0.3.16 版本更新：回到前台也会自动检查更新，更新提醒不再错过；"
    "“稍后再说”超过 24 小时自动解除。包含 0.3.15 全部修复。"
)
