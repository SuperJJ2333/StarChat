"""发布 0.3.21+24 应用更新设置（latest_build 采用 arm64 清单值 2000+build，兼容旧客户端构建号比较）（在 starchat-business-api-1 容器内执行）。

用法：
  docker exec -i starchat-business-api-1 python - < publish_app_update_0.3.21.py
...
IDEMPOTENCY_KEY = "app-update-publish-0.3.21-20260831"
APK_URL = f"https://www.liuhetong888.com/downloads/ChatFlow-{RELEASE_VERSION}-arm64.apk"
NOTES = (
    "0.3.21 版本更新：1) 表情面板全新三栏图标切换（表情/超级表情/我的表情），"
    "文字与表情混排输入、发送后及历史消息中的表情持续播放动态效果；"
    "2) “更多”面板新增可扩展“工具”面板；3) 修复图片发送对 MKV/AVI 等"
    "视频格式的支持，图片预览与选中交互分离；4) 聊天中图片消息补齐头像与"
    "昵称，会话列表摘要显示 [图片]/[语音]/[视频]/[语音通话]/[视频通话]；"
    "5) 好友名称备注优先并支持全界面即时刷新；6) 群聊导航标题显示"
    "“群名（人数）”；7) 修复会话退出后未读角标残留。"
)
