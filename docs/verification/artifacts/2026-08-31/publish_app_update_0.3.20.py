"""发布 0.3.20+23 应用更新设置（latest_build 采用 arm64 清单值 2000+build，兼容旧客户端构建号比较）（在 starchat-business-api-1 容器内执行）。

用法：
  docker exec -i starchat-business-api-1 python - < publish_app_update_0.3.20.py
...
IDEMPOTENCY_KEY = "app-update-publish-0.3.20-notes2-20260831"
APK_URL = f"https://www.liuhetong888.com/downloads/ChatFlow-{RELEASE_VERSION}-arm64.apk"
NOTES = (
    "0.3.20 版本更新：新增端到端加密语音通话与视频通话（微信式界面：来电"
    "拒绝/接听、通话时长、静音/免提/切换摄像头）；后台与锁屏来电全屏提醒，"
    "通话中切后台仍可继续；主叫 60 秒无应答自动取消；修复旧版本收不到"
    "更新提醒的问题。"
)
