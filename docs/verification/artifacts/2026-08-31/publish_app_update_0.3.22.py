"""发布 0.3.22+25 应用更新设置（latest_build 采用 arm64 清单值 2000+build，兼容旧客户端构建号比较）（在 starchat-business-api-1 容器内执行）。

用法：
  docker exec -i starchat-business-api-1 python - < publish_app_update_0.3.22.py
...
IDEMPOTENCY_KEY = "app-update-publish-0.3.22-notes2-20260901"
APK_URL = f"https://www.liuhetong888.com/downloads/ChatFlow-{RELEASE_VERSION}-arm64.apk"
NOTES = (
    "0.3.22 版本更新：表情面板三栏图标切换，文字与表情混排持续播放动画；新增可扩展工具面板；修复 MKV/AVI 等视频发送与 GIF 动画播放；语音消息支持暂停/继续；通话结束自动返回会话页；未读角标与好友备注展示优化；安装包完成安全加固。"
)
