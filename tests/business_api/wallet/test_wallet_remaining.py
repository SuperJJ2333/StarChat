...
def test_withdrawal_confirmation_webhook_is_idempotent(wallet):
    ...
    row = service.request_withdrawal(user_id="u1", amount=Decimal("10.000000"), address="T1", client_order_id="order-1", reason_code="USER_WITHDRAWAL")
    service.finance_approve(row.id, "finance")
    service.admin_approve(row.id, "admin")
    service.submit_to_custody(row.id, "finance")
    event = provider.withdrawal_event(client_order_id=row.id, status="CHAIN_CONFIRMED", confirmations=20, event_id="wd-event-1")
...
def test_unknown_withdrawal_result_is_queried_by_original_order(wallet):
    ...
    row = service.request_withdrawal(user_id="u2", amount=Decimal("10.000000"), address="T2", client_order_id="order-2", reason_code="USER_WITHDRAWAL")
    service.finance_approve(row.id, "finance")
    service.admin_approve(row.id, "admin")
    service.submit_to_custody(row.id, "finance")
