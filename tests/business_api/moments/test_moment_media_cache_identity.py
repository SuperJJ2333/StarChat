...
def test_resigning_preserves_deployed_reference_digest_contract(media_context):
    ...
    # Release2077 accepts only completed uploads owned by the author.
    with pytest.raises(AppError) as external:
        service.create('alice', {'text': 'synthetic', 'visibility': 'PUBLIC',
                                'image_urls': ['https://external.example/photo?v=one']}, 'invalid-post')
    assert external.value.code == 'MOMENT_MEDIA_INVALID'
    assert external.value.status_code == 422
    created = service.create('alice', {'text': 'synthetic', 'visibility': 'PUBLIC',
                                      'image_urls': [original]}, 'post')
    replacement = _uploaded(media, key='replacement')
    with factory() as session:
        ...
        row.image_urls = ['media://' + replacement.object_key]
        ...
        assert unknown['image_urls'][1] == ''  # Invalid legacy references grant no media capability.
