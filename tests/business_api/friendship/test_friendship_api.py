...
async def test_friend_requests_and_search_return_business_profile_projection(ctx):
    ...
    async with AsyncClient(transport=ASGITransport(app=app), base_url='http://test') as client:
        ...
        assert request_item == {'id': created.json()['id'], 'user_id': 'u1', 'username': 'alice', 'nickname': 'Alice', 'avatar_url': 'https://media.example.test/avatar.png?signed=1', 'matrix_user_id': '@alice:matrix.example.test', 'message': '你好', 'remark': None, 'tags': [], 'status': 'PENDING', 'direction': 'INCOMING', 'requested_at': request_item['requested_at']}; assert request_item['requested_at']
